...
- **Multi-account fleet tooling (opt-in, Linux & macOS)** — per-chat `/swap` reboots a running chat onto a different Claude subscription in place (same pane, other sessions untouched); ships the reboot contract, not the respawn engine — you author that part, see `blueprint/templates/host-swap/swap.command.md`.
