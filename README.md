...
Opt-in, and the most unusual thing in the box: `pfm` treats _chats_ as infrastructure. The engine and installer live under `pfm/`; every staged command card, helper, launcher shim, and scheduler unit — systemd on Linux, launchd on macOS — has one source under `pfm/internal/installer/assets/` and is embedded in the binary.
...
- **`/clear` auto-hide** — clearing a Claude or Codex chat hides the finished session; Codex completes the hide when the first prompt in the fresh chat begins, which is when its clear event fires. If the finished conversation is resumed and speaks again, it becomes visible automatically. `/exit` remains a plain exit.
- **`pfm chat` + `/chat:*`** — the per-chat interface and its instruction cards. `inject` types a real turn into another pane under a per-target lock, protecting drafts and refusing unsafe dialogs; `new` starts an immediately addressable named chat on an immutable socket; `read`, `stream`, `capture`, `name`, `hide`, `end`, and the group verbs share one target resolver. `/chat:branch` creates a detached fork without moving the current pane, and `/chat:interrogate` resumes a finished session to ask it _why_.
- **Keeping the fleet healthy** — `pfm heal` reports or repairs a wedged Codex history projection: the failure where a resumed thread renders as if it were brand new because its projection cursor desynced from the rollout. It is read-only until asked, backs up each store before it deletes a row, and lets Codex rebuild the projection from the rollout itself. Beside it: `reap` classifies the socket graveyard and reclaims it under `--apply`, `archive` moves hidden chats and old subagent transcripts out of sight reversibly, `revive` lists resumable chats by project, `index` refreshes the transcript index, `doctor` inspects database and jail health, and `dream` builds and injects a repository's memory organs. The wiring verbs — `statusline`, `usage-hook`, `name-sync`, `mcp` — are called by hooks and units rather than typed; `pfm help` lists every one.

**Read before opting in:** the fleet assumes `tmux`, `zsh`, `fzf`, and `jq`, runs on Linux and macOS (the process table is `/proc` on Linux and `sysctl` on macOS; the name-sync scheduler is systemd user units or a launchd agent) with Windows unsupported — and it launches chats with permission prompts disabled by explicit design, leaving PreToolUse hooks as the remaining brake. That trade-off is documented in the bundle, not hidden; make it consciously.
