...
3. **Verification evidence** — per merged wave: the reviewer verdict artifact (the wave dir's `REVIEW.md` ledger) exists and covers the merge candidate; the builder's suite log is real and green; the post-merge test event is in the ledger. A merge without its reviewer verdict is a protocol breach, not a gap.
