...
**Command-owned (`docs/commands/{cmd}/`):** `documenter/references/sync-rules.md`, `documenter/references/doc-approval.md`, `documenter/references/scopes/*.md` → `/documenter`; `ptm/references/` → `/ptm`; `officer/references/` → `/officer`; `mentor/references/` → `/mentor`; `pm/references/` → `/pm`; `km/research/` → `/km`; each other opted-in Tier B command owns its `references/`/`research/` directory.
...
**Ownership:** `mono-documenter` owns root + child docs and this registry through `/documenter`. `/pm` owns PM references. `/officer`, `/mentor`, `/km`, `/ptm` own their command reference/research directories. `.claude/` and `.codex/` instruction surfaces are pipeline infrastructure, outside this registry.
