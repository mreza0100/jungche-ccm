...
# Session-keyed guard-marker maintenance (ptm/km gates).
...
#                    (ptm-guard's sliding 1500s freshness + the 1h reap here), so a
...
reap() {
  local root="$1" now
  now=$(date +%s)
  for m in "$root"/tmp/professor_ptm_active* "$root"/tmp/professor_km_active* "$root"/tmp/professor_quality_loaded*; do
    [[ -f "$m" ]] || continue
