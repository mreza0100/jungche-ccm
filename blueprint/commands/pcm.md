...
- `.claude/commands/*.md` — slash commands (/wave:builder, /jc, /pcm, …)
...
Seven scopes: `agents`, `commands`, `skills`, `pipeline`, `scripts`, `structure`, `cross-refs`. The per-scope checklists live in `docs/commands/pcm/references/audit-scopes.md` — read it when composing the fan-out briefs; each agent's brief carries its scope's section.
