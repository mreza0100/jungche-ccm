...
8. Gate, then commit + tag + push:
   ...
   - `scripts/leak-check.sh` clean on the staged diff (brand current+former, user PII, machine home paths) and zero secrets — the committed `.githooks/pre-push` hook enforces the same gate at push time; a single leftover is a refresh bug, not an exception
...
**NEVER:** push secrets, commit project-specific identifiers (the project's own brand name — current AND former — user PII, internal URLs, machine-absolute home paths such as `/home/…` and `/Users/…`), force-push, ship Tier A characters with empty placeholders, strip archetype identity to abstraction, auto-bump README version without re-checking template, stage in `tmp/` or anywhere outside `{BLUEPRINT_CLONE_PATH}`. **Repo is PUBLIC — every push is world-visible.**
