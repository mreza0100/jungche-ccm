...
7c. **Installs statusline** — obtains the `pfm` binary and runs `pfm install --apply`, which merges a `statusLine` command running `~/.local/bin/pfm statusline` into `~/.claude/settings.json`. The native renderer shows model, fleet counts, context, git, cache state, cost, spend, and rate limits; detached refreshers keep network work off the render path.
...
7g. **(Opt-in) Installs multi-account fleet tooling** (Linux and macOS — plain per-account config dirs, no Keychain) — with the user's consent (it edits global shell config and `~/.claude/`), gets the `pfm` binary and runs `pfm install --apply`. Building from the permanent clone with `go -C "$HOME/.professor/pfm" build -o "$HOME/.local/bin/pfm" ./cmd/pfm` is one way to obtain it; `pfm install --dry-run` previews the exact changes. The self-contained binary embeds every host asset from `pfm/internal/installer/assets/`: `/bb`, `/swap`, and `/chat:*` instruction cards and helpers, the launcher shim, and the name-sync units. Its installer also wires the native statusline, usage, BB, group, and dream hooks across existing canonical account settings. A real file at a destination is backed up, never destroyed; `pfm install --uninstall` reverses it. Skipped if the user declines.
