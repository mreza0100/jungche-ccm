...
7g. **(Opt-in) Installs multi-account fleet tooling** (Linux and macOS — plain per-account config dirs, no Keychain) — with the user's consent (it edits global shell config), appends `blueprint/templates/host-swap/zshrc-swap.snippet.sh` to `~/.zshrc` (the `cc`/`cc1`/`cc2`/`cc3` launchers + `cc-swap` picker), copies `swap.command.md` to `~/.claude/commands/swap.md` (the `/swap` reboot-in-place *contract* — the adopter still authors the engine it shells out to; see that file), and (if using the Professor statusline) merges `statusline-badge.snippet.sh` into the statusline script. The adopter MUST first edit the account table (labels/account count) per `blueprint/templates/host-swap/README.md`. Skipped if the user declines.

7g-i. **(Opt-in, host-level) Installs the `/chat:*` family** — copies `blueprint/templates/host-swap/chat/` to `~/.claude/commands/chat/` (each `*.command.md` renamed to `*.md`, `chat.sh`/`history.sh` chmod +x'd — see INSTALL.md § Step 8.4a for the exact commands). Installed to the user's HOME, deliberately, not the repo: `~/.claude/commands/` resolves the same in every project and every worktree, which is exactly why `chat:*` lives there — a project-level copy is a set of relative symlinks that break inside a worktree at a different directory depth. Joins `/bb` and `/swap` as the third host-level command family.
