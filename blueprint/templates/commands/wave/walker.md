...
## Engine profile (`args.project`) — written once at initiation

The engine bundle is **universal** — one build serves every project; nothing project-specific is baked in. Everything project-specific arms at runtime from `args.project`, and this section is its single home: install (or adoption) writes the block below **once**, and every invocation in § Entry points passes it verbatim as `args.project`, all modes. Never edit the bundle itself — it opens with a `GENERATED FILE — DO NOT EDIT` banner and any hand edit is overwritten by the next build (source of truth: `~/.professor/ENGINES/wave-walker/engine`, `npm run build`).

Absent profile — or an invalid gate regex — the gate machinery (gate sweeps, R6/R7, fence rules) reports `gates: SKIPPED — no project profile supplied` (or `— invalid gate pattern: <err>`) in Coverage and telemetry: loud, never silent. The thread walk, security fan-out, hygiene, and panel modes run fully regardless.
