...
- **An error never renders as ABSENCE** — absence is a claim about the world ("no data exists"); an error is a claim about ourselves ("we failed to look"). Every empty/no-data/degraded state — UI, health check, gate verdict — distinguishes the two, and a {DOMAIN_ADJ} surface that shows "no signal" while the signal sits in the DB is a silent false negative on {DOMAIN_ADJ} data: the {USER_NOUN} is misled by a screen that never admits it broke. Logging the error is necessary and NOT sufficient; the visible state must tell the truth. Nor may an error render as a DIFFERENT error: an uncancelled timeout whose once-guard lets a second caller re-enter surfaces as an auth failure, sending the debugger to the wrong subsystem entirely. **The test:** ask what this mechanism would report if it were BROKEN — same answer as "nothing here", or as some unrelated failure? That is the bug, found before it reaches a {USER_NOUN}.
...
<!-- KEEP the next rule only if the roster includes an AI/LLM project with its own prompt-template directory; drop it for a roster with no such project. -->

- **No inline LLM prompts in code** — every LLM-bound prompt lives in `{AI_PROJECT}/knowledge/prompts/` as `.md`, loaded via `load_prompt()`. Detail: `{AI_PROJECT}/.claude/agents/ai-engineer.md`.
...
- **Two environments:** `.env.local` (dev, port {DB_PORT}/{QUEUE_PORT}) and `.env.test` (test, port {DB_PORT_TEST}/{QUEUE_PORT_TEST}). Start infra: `make -C {INFRA_PROJECT} up-local` / `up-test`. **`.env.test` only for integration/e2e** — QA blocks (`BUG-WRONG-ENV`) any test that loads the wrong-tier env as its primary.
...
- **A test reaches production or it is not a test** — import and call the symbol the test names; assert what production RETURNED, never a value the test declared. A test that rebuilds the structure under test (its own resolver map beside the one the server assembles), re-implements the branch inline, or compares a literal to itself is green against broken code by construction. The test name is a claim its body must honour.
- **Tier by boundary, not by directory** — a test reaching no real DB/queue/HTTP belongs in unit; integration means a real boundary is crossed. Fixture indirection hides boundaries — resolve the fixture graph, never the argument list.
- **One number, one home** — a count duplicated across files drifts, and each copy earns its own green test pinning its own copy. Derive the invariant (`sorted(actual) == sorted(declared)`); never assert the literal.
- **Parallel test execution is a test-health invariant** — every project's `{PROJECT_TEST_RUNNER}` always runs at its fixed parallelism (e.g. `--parallel 4`, `-n4`); never lower workers to pass. A test failing under parallel execution is unhealthy — make it parallel-safe (per-worker DB clone, mocked external-service port, isolated storage bucket, self-contained scenarios).
