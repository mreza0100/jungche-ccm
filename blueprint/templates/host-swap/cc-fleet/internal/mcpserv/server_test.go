...
func setupBackendFixture(t *testing.T) string {
	...
	t.Setenv("CC_FLEET_CODEX_AVAILABLE", "0")
	return root
}
...
func buildFleetBinary(t *testing.T, destination string) string {
	...
	binary := filepath.Join(destination, "cc-fleet-test")
	command := exec.Command(
		"go",
		"build",
		"-trimpath",
...
func TestMCPStressSequentialCallsNoLeaks(t *testing.T) {
	...
	if os.Getenv("CC_FLEET_STRESS_STRICT") == "1" && elapsed > 5*time.Second {
...
func TestMCPStressConcurrentEightClientsNoCrossTalk(t *testing.T) {
	...
	elapsed := time.Since(started)
	if os.Getenv("CC_FLEET_STRESS_STRICT") == "1" && elapsed > 8*time.Second {
