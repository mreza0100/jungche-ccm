# cc-fleet — Go rewrite of the cc-ls chat-fleet tooling
...
**Ground rule:** until WP11 you create/modify files ONLY under `~/work/host-ops/cc-fleet/`.
The living zsh (`oldbox/scripts/cc-fleet.zsh` and satellites) stays untouched and in service.

## Why

`cc-ls` today is `oldbox/scripts/cc-fleet.zsh` — 1,293 lines of zsh + fzf listing every live and
...
daemon), greenfield in `cc-fleet/`, fancy Bubble Tea TUI.
...
- **K1 — eval protocol.** The binary never execs the final tmux attach. TUI renders on
  ...
  `cc-ls() { local out; out="$(cc-fleet ls "$@")" || return $?; [[ -n "$out" ]] && eval "$out"; }`
...
- **K4 — test jail.** All tests: `TMUX_TMPDIR = t.TempDir()`; real paths overridable via env
  `CC_FLEET_DB / _SID_DIR / _CLAUDE_ROOTS / _CODEX_ROOT / _TMUX_DIR / _HOME` (test-only, not a
...
```
cc-fleet/
  go.mod                      # module hostops/cc-fleet
  cmd/cc-fleet/main.go        # stdlib flag dispatch
  ...
  shim/cc-fleet.zsh           # post-cutover thin wrapper (WP10; wired only at WP11)
  testdata/                   # claude-store/ codex-store/ crumbs/ proc/ golden/
```

## SQLite schema + DB policy

DB: `~/.local/state/cc-fleet/fleet.db` (spans 3 accounts + codex → not under ~/.claude).
...
```
cc-fleet ls [-a|--all] [-H|--hidden] [<uuid>] [--plain|--tsv|--check]
cc-fleet open <uuid>                     # cc-open port; stdout = eval line
cc-fleet index [--full] [--progress]
cc-fleet hide [--self|<id>] [--exit]  ·  unhide <id>  ·  hidden
cc-fleet resolve label|session|cxwin <name>   # chat.sh contract: "sockpath\tpane" rc 0/1/2, candidates → stderr
cc-fleet revive  ·  legacy import|export  ·  doctor  ·  version
cc-fleet mcp                             # stdio MCP server — the /chat:* family as tools (WP12)
cc-fleet internal hide-exit …            # hidden: detached --exit finisher (setsid re-exec)
```
...
`cc-hide.sh`/`cx-hide.sh` become 2-line delegators to `cc-fleet hide --self [--exit]`; their
...
in `shim/cc-fleet.zsh`: `cc`/`cc1-3`/`cx`, `_cc_run`, `_cx_server`, `_cc_arm1h`, `_cc_primary`,
`_cc_in_bunker`, `cc-swap`, `vsct-revive`, + the one-line eval wrappers. `~/.claude-primary`
and `/tmp/cc-sid` crumbs stay plain files (statusline-command.sh, cc-reap, cc-swap-chat,
...
1. **Shadow**: `cc-fleet ls --check` runs `zsh -ic 'cc-ls -a'` with fzf masked from PATH
...
2. **Import**: `cc-fleet legacy import` — cold `index --full`, then hidden + `.at` conversion
...
3. **Cutover (WP11 — the ONLY step touching existing files; gitter commits):** flip
   `~/.zshrc:56` to source `cc-fleet/shim/cc-fleet.zsh`; replace the two hide scripts with
   delegators; update `docs/cc-fleet.md` (drafted in `CUTOVER.md` beforehand). VS Code tasks
...
- **WP0 — toolchain + skeleton.** `mise use -g go@1.24`; `go mod init hostops/cc-fleet`;
  `cmd/cc-fleet` dispatch + `version`; package stubs; `internal/paths` env-override helper (K4);
  `testdata/` scaffold. Accept: `CGO_ENABLED=0 go build ./...` produces a static binary;
  `cc-fleet version` prints.
...
- **WP9 — cli wiring.** End-to-end in a fully jailed env (fixture stores + jailed tmux + fake
  ...
  supervisor.
- **WP10 — shadow + shim.** `ls --check`; write `shim/cc-fleet.zsh`; draft the docs update as
...
- **WP12 — MCP server (the /chat:* family as tools).** `cc-fleet mcp`: a stdio MCP server via
  ...
  draft → allowed) green. Registration at cutover+: `claude mcp add cc-fleet -- cc-fleet mcp`
  + codex `config.toml` `mcp_servers`; chat.sh itself stays untouched and working.

**Final acceptance (supervisor, live box):** warm `cc-fleet ls --plain` < 1 s wall; `--check`
...
- `oldbox/scripts/cc-fleet.zsh` — the behavioral source of truth (row composition, keybinds,
