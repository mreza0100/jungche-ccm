...
// bug: `cc-fleet hide <id>` only accepted an id the store already indexed,
...
// `cc-fleet hide` in-process exactly as the CLI does — no fake ProcFS or
...
// CC_FLEET_PROC_ROOT: cmdline and environ as NUL-delimited records, and a
...
// newHideCLIJail wires the CC_FLEET_* env exactly like jailTest, except the
// tmux directories agree with each other (TMUX_TMPDIR's parent equals
// CC_FLEET_TMUX_DIR, tmux's own "tmux-<uid>" convention) so a REAL
...
func newHideCLIJail(t *testing.T) *hideCLIJail {
	...
	t.Setenv("CC_FLEET_SID_DIR", sidDir)
	t.Setenv("CC_FLEET_CLAUDE_ROOTS", claudeRoot)
	t.Setenv("CC_FLEET_CODEX_ROOT", codexRoot)
	t.Setenv("CC_FLEET_TMUX_DIR", tmuxDir)
	t.Setenv("CC_FLEET_PROC_ROOT", procRoot)
