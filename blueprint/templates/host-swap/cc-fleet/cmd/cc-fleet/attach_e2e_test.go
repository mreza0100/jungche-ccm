...
const attachHelperEnv = "CC_FLEET_ATTACH_HELPER"

// TestCCFleetAttachHelper turns the package test binary into the real CLI for
...
func TestCCFleetAttachHelper(t *testing.T) {
...
func TestJailedEvalAttachFromPlainAndNestedTmux(t *testing.T) {
	...
	index := exec.Command(filepath.Join(jail.home, ".local", "bin", "cc-fleet"), "index", "--full")
	index.Env = jail.env
...
func newAttachJail(t *testing.T) *attachJail {
	...
	return &attachJail{
		...
		id:         id,
		env:        env,
		shim:       filepath.Clean(filepath.Join(cwd, "..", "..", "shim", "cc-fleet.zsh")),
	}
}
...
func (jail *attachJail) proveAttach(
	...
	commandText := `cc-fleet open ` + shellQuote(jail.id)
	if flow == "picker" {
		commandText = `cc-fleet ls`
	}
	if protocol == "eval" {
