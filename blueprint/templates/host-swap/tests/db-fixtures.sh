...
ok "two hidden" "$(bash "$CC" hidden-list | wc -l)" "2"
...
ok "after delete" "$(bash "$CC" hidden-list | wc -l)" "1"
bash "$CC" hidden-add aaaaaaaa-0000-0000-0000-000000000002 >/dev/null
ok "re-add is not a duplicate" "$(bash "$CC" hidden-list | wc -l)" "1"
...
wait
ok "all 20 concurrent hides survived" "$(bash "$CC" hidden-list | wc -l)" "20"
...
ok "db beats legacy under concurrency" "$([ "$(bash "$CC" hidden-list | wc -l)" -gt "$legacy_kept" ] && echo yes || echo no)" "yes"

echo "=== 4. hidden-reap is one transaction ==="
printf 'bbbbbbbb-0000-0000-0000-000000000001\nbbbbbbbb-0000-0000-0000-000000000002\n' | bash "$CC" hidden-reap
ok "reap kept only the live set" "$(bash "$CC" hidden-list | wc -l)" "2"
...
ok "pane children for sess-1" "$(bash "$CC" child-list pane sess-1 | wc -l)" "2"
ok "new children are a separate kind" "$(bash "$CC" child-list new sess-1 | wc -l)" "1"
bash "$CC" child-add pane sess-1 cc-sock-a >/dev/null
ok "duplicate child ignored" "$(bash "$CC" child-list pane sess-1 | wc -l)" "2"
...
ok "reads legacy file when db absent" "$(bash "$CC" hidden-list | wc -l)" "1"
...
ok "hidden migrated" "$(bash "$CC" hidden-list | wc -l)" "2"
...
ok "children migrated" "$(bash "$CC" child-list pane s1 | wc -l)" "1"
...
bash "$CC" migrate >/dev/null 2>&1
ok "second migrate is a no-op" "$(bash "$CC" hidden-list | wc -l)" "2"
...
ok "two chat rows" "$(bash "$CC" chat-load | wc -l)" "2"
...
bash "$CC" chat-prune
ok "prune drops rows with no transcript" "$(bash "$CC" chat-load | wc -l)" "0"
