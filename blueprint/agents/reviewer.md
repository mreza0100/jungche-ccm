...
**TEST seat** (`general-purpose`, in TREE only): run the project's own gate (`cd {project} &&
{PROJECT_TEST_RUNNER}` if present, else the language's native test command) UNPIPED, watched to
