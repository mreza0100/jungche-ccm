...
14. `/jc` scrub the two `~/work/limit-dashboard`-style lines to a genericized path, and widen `leak-check.sh`'s PATTERN with a `~/...` home-relative alternative so this class of leak is caught mechanically. — `docs/dev/trains/queue/2026-08-21-limits-live-fetch.md:5`, `docs/dev/trains/pfm-wave-2/STATE.md:66`, `scripts/leak-check.sh`
