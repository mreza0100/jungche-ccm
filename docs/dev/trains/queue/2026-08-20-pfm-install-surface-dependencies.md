# Wave: pfm-install-surface-dependencies

**Status:** SCHEDULED → pfm-wave-2 (2026-08-20)
**Refined:** 2026-08-20 · main @ `7b01caa` (tree carries the uncommitted #1–#17 span; anchors below verified against the working tree, not the commit)
**Epic:** none
**Touches:** pfm, docs (README/INSTALL), CI
**Scope:** the three production surfaces Task #11's e2e harness relies on, ruled by the founder 2026-08-20: the install/uninstall CLI verb redesign, doctor clean on a fresh install, and an apply-time harvestpy skip path. These are the dependency tasks the original refinement asserted as already existing (phantom anchors — see pfm-wave-2 `train.md` § Source Reconciliation and the sentinel audit); #11's e2e suite cannot go green until they exist.
**Numbering:** pfm-wave-2's number space, extended — #18, #19 are taken (F1, F4); these are **#20–#22**.
**Deferred:** nothing new; every deferral of the source spec stands.
