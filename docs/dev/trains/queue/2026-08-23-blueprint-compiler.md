# Blueprint compiler: `blueprint/**` is the only source; pfm compiles it through two middleware layers into every engine, per project and per machine

Status: QUEUED · Refined: 2026-08-23 by Professor (user rulings § 2) · Project: pfm + blueprint + docs · Fenced waves (four milestones, one worktree) · **Depends on `2026-08-23-engine-contract.md` — merged to `main` first**; this spec consumes its `internal/engine/**` registry and never spells an engine name outside it.
...
3. **Install copies, so update must merge.** `blueprint/**` templates carry `{TOKENS}`; `docs/SETUP.md` Phase 2 has an LLM fill them and copy files into `.claude/`; `/pcm:update` (`blueprint/commands/pcm/update.md`) is an LLM three-way hash merge per file. Every local edit is a future merge conflict; every update is a language-model judgment over prose.
4. **This repo keeps two copies of every prompt equalized by an LLM.** Live `.claude/**` is the source; `blueprint/**` is re-derived by `scripts/refresh-scope.sh` + `blueprint/refresh-map.json` (631 lines of hashes) — "UNCHANGED hashes are a mechanical untouched-proof — skipped" — and the rest is LLM re-derivation. The 24 files present in both trees all differ (`diff blueprint/commands/git.md .claude/commands/git.md` = 21 lines; `wave/live.md` = 77). `.professor/drift.md` carries 16 `KEEP-LOCAL` bullets explaining, in prose, why.
...
6. **Placeholder surface, measured.** Tokens per template (`grep -oE '\{[A-Z][A-Z0-9_]+\}' | sort -u | wc -l`): `commands/git.md`=0, `wave/builder.md`=0, `wave/orchestrator.md`=0, `wave/ccc.md`=1, `wave/live.md`=1, `p/tokens/SKILL.md`=0, `p/slow-burn.md`=0, `sleep.md`=0, `goal-manager.md`=0, `animate.md`=0, `agents/scheduler.md`=0, `agents/architect.md`=0, `agents/tracer.md`=1, `agents/gitter.md`=2, `pcm.md`=2, `wave/refine.md`=5, `quality/prompt.md`=6, `wave/walker.md`=9, `dev.md`=19, `scripts/dev.sh`=36, `output-styles/professor.full.md`=23. No `commands/h/**` exists in `blueprint/` or `.claude/` (it is an `ignore_sources` entry in `refresh-map.json`, i.e. a private-source command never shipped) — there is nothing `/h:*` to globalize; § 5.4 records this.
...
| --- | --- | --- |
| **store** | an immutable checkout of `blueprint/**` at one release tag | `$PFM_DATA/professor/store/<tag>/` (`$PFM_DATA` = `~/.local/share/pfm`); `current` symlink → the active tag. **Self-hosted** (`manifest.installed_from.mode == "self-hosted"`): store = `<root>/blueprint/`, version = `VERSION` + `git rev-parse HEAD` |
| **adapter** | shipped middleware for one engine: `engine.json` + override files | `blueprint/adapters/<engine>/` |
...
| **template metadata** | per-template scope/tier | `blueprint/templates.json` (§ 4.3) |
| **placeholder registry** | token → manifest path | `blueprint/placeholders.json` (§ 4.2) — the machine form of `docs/PLACEHOLDERS.md`, which remains the human form and must agree |
...
```
Generated by pfm professor build from {src}@{ver} — DO NOT EDIT. Framework change → edit blueprint/{src} and rebuild. Project customization → write an override under .professor/overrides/ (pfm professor override --help).
```
...
```
professor: SOURCE TEMPLATE — edit here for a framework change (routes through /pcm); a project-only customization is an override under .professor/overrides/, never an edit to a generated copy.
```
...
### 4.2 `blueprint/placeholders.json`
...
**Key behaviors:**
- a. `store.Open(root, manifest)` → `Store{Path, Version, Mode}`; tag mode reads `$PFM_DATA/professor/store/current`; self-hosted reads `<root>/blueprint` + `VERSION` + `git rev-parse --short HEAD` (a missing `.git` is `self-hosted@unknown`, reported, never fatal).
...
**Key behaviors:**
- a. `pfm professor store fetch [--to vX.Y.Z]` — from the source clone recorded by `installer.ReadSourceRepoMarker` (reuse `update_command.go`'s `selectHighestSemver`, `updateGitRun`): `git archive <tag> blueprint` → `$PFM_DATA/professor/store/<tag>/` (atomic: extract to `<tag>.partial`, rename); `current` symlink switched only after extraction verified (`templates.json` parses). A tag without `blueprint/templates.json` is `STORE-PRE-COMPILER {tag}` — refused; the user is told the first compiler-aware release.
...
#### Task 2.3 — `pfm init` writes the manifest skeleton; `/pcm:update` shrinks to the interview delta

**Key behaviors:**
- a. `pfm init [dir]` creates `.professor/manifest.json` with every `required: true` token's answer path present and `null`, `engines: ["claude"]`, `overrides.tracked: false`, `scope.project: []`, `installed_from` = store mode+version; prints `open Claude here: /pcm:install` (the interview). It no longer copies a file scaffold (`initScaffold`, `copyInitTree`, `copyInitFile` deleted; `discoverSourceRepo`/`isSourceRepo` stay for the source-marker fallback).
- b. `blueprint/commands/pcm/update.md` is rewritten: it runs `pfm update --json`, reads the report, and does exactly two LLM things — asks the interview question for each `missing-answer` token (writing the answer into the manifest) and presents each `held` file with its one clearing action; then re-runs `pfm professor build`. The three-way merge prose is deleted entirely. `blueprint/commands/pcm/install.md` (new) is the interview: SETUP.md Phase 1 questions → manifest, then `pfm professor build`.
- c. `docs/SETUP.md` Phase 2 (LLM customization), Phase 3's copy steps, and `docs/BLUEPRINT.md` § "Staying current" are rewritten to describe § 3 (the builder rewrites; the user reviews at the checkpoint). `INSTALL.md` paths 1 and 2 end in `pfm init` + `/pcm:install`.

**Publication surface:** `blueprint/**`, `INSTALL.md`, `docs/*.md` — invented example values only; run `scripts/leak-check.sh` before the checkpoint.
...
**Key behaviors:**
- a. Stamp the source marker (§ 3.5) on every template; write `blueprint/placeholders.json` (from `docs/PLACEHOLDERS.md`, complete), `blueprint/templates.json` (every template; scopes per § 5.4 table), `blueprint/adapters/{claude,codex,opencode}/engine.json` + override files (§ 4.4; content lifted verbatim from `.claude/codex-build.json` and `build-opencode.mjs`'s permission block).
- b. Convert representative-pattern templates into `{{#each roster}}` blocks: the set is every template `docs/PLACEHOLDERS.md` § Project roster names as roster-tokenized (grep the roster tokens of § 4.2 across `blueprint/`); each conversion is byte-verified by building against this repo's manifest and diffing against the live file (Task 3.2 makes that diff the acceptance test).
- c. `blueprint/refresh-map.json`, `scripts/refresh-scope.sh`, `blueprint/commands/pcm/references/refresh.md`, `docs/commands/pcm/references/refresh.md`, `blueprint/scripts/build-codex.mjs` — **deleted**, with every reference scrubbed (`README.md`, `docs/BLUEPRINT.md`, `docs/SETUP.md`, `blueprint/commands/pcm.md`, `blueprint/commands/pcm/release.md` step that regens the map, `.claude/commands/pcm/release.md` via its template).
...
| --- | --- |
| `agents/dev.md`, `agents/qa.md` (global dev/qa, no per-project pair) | (ii) SHIP as `blueprint/agents/dev.md`, `qa.md`; `templates.json` marks `per-project/developer.md`/`qa.md` as the `per: roster` alternative selected by a new manifest answer `agents.layout: global|per-project` (this repo: `global`) |
...
| "no `/pcm:update` here" | selection: `scope.drop: ["commands/pcm/update.md"]` via a `drop-file` override |
...
| `explore-deny.sh`, `codex-build.json` | `explore-deny.sh` → (ii) SHIP under `blueprint/scripts/`; `codex-build.json` → deleted (Task 3.1 adapters replace it) |
...
| `.claude/skills/rr/**` (gitignored, source-fetched) | untouched — `skills/sources.json` law stands |
...
**Key behaviors:**
- a. `blueprint/settings.json` Stop hook: `codex-sync.sh` → runs `pfm professor build --scope project` when the turn edited `blueprint/**` or `.professor/overrides/**` (the mark/sync split stays: `codex-sync.sh mark` on PostToolUse, `sync` on Stop), then `pfm professor check`; a non-zero `check` prints its § 6 report into the session (the hook's stdout), never swallowed. Bash-driven writes remain outside hook coverage; `check` in the pcm `structure` audit is the backstop (keep the comment that says so).
- b. `pcm-guard.sh`: guarded set becomes `blueprint/**` + `.professor/overrides/**` (sources) AND the generated trees (`.claude/**`, every `CLAUDE.md`/`AGENTS.md`, `.codex/**`, `.opencode/**`) with a *different* deny text for generated files: `GENERATED by pfm professor build from blueprint/{src} — edit the template (framework) or write an override under .professor/overrides/ (project), then rebuild. Hand edits are detected and held.` The `/pcm` + `quality/prompt.md` unlock continues to govern the sources.
- c. Root `CLAUDE.md` template: § Three-runtime team rewritten around the compiler (one paragraph; the `build-codex.mjs`/`build-opencode.mjs` commands and the "AGENTS.md is compiled from this file" sentence are replaced by `pfm professor build`/`check`); § Repo structure's `.claude/` line becomes "generated from `blueprint/` — source of truth is the blueprint"; `scripts/refresh-scope.sh` mention removed. ≤ 200 lines.
...
1. In the worktree: `pfm professor build --scope project --root .` (self-hosted store = the worktree's `blueprint/`). Exit must be `0` — a hold here is a reconciliation miss: fix the template/override, never the output.
...
- c. Global agents travel with global commands: `gitter`, `scheduler`, `tracer`, `frr`(already), `architect` → `~/.claude/agents/`, `~/.codex/agents/*.toml` (existing `globalagents` path), `~/.config/opencode/agent/`. A global command's `subagent_type` must resolve to a global agent — `pfm professor check` greps every `subagent_type:` in a global command against the global agent set and reports `GLOBAL-DANGLING-AGENT {cmd} → {agent}` (§ 1.7: cross-scope dispatch is undocumented; we do not rely on it).
...
`manifest.scope.project: []`; build `--scope all`; `.claude/commands/` retains only project-scoped templates (`dev.md`, `quality/*`, `wave/refine.md`, `wave/walker*.md`, `pcm/release.md`, `p/tokens/README.md`); the rest now resolve from `~/.claude/commands/`. `pfm doctor` shows both scopes. `/git`, `/wave:ccc`, `/wave:live`, `/pcm` invoked from this repo read identically (byte-diff the global output against the pre-M4 project output: only the marker's `scope` word differs).
...
professor: /path/to/project  store v0.54.0 → v0.55.0  engines claude,codex
  ...
  held          2
    ...
    CLAUDE.md                            missing-answer {DOMAIN_RISK_EXAMPLE}
                                         action: answer it — /pcm:update asks the question, or edit .professor/manifest.json answers.domain_risk_example
...
- `skills/sources.json` source-fetched skills (`rr`) — unchanged law.
- Releasing: `/pcm:release` sequence adjusts only in what it deletes (Task 3.1.c); `VERSION`/`CHANGELOG` discipline unchanged.
...
- A template's token is absent from `docs/PLACEHOLDERS.md` (the human registry must be updated by `/pcm`, not by you inventing a token).
