...
6. **`/ptm` is renamed `/pfm`** (2026-08-29, user-ruled): every `/ptm` reference in this spec reads `/pfm` — the command card, its subcommands (`/pfm:release`, `/pfm:install`), `ptm-guard.sh` → `pfm-guard.sh`, `docs/commands/ptm/` → `docs/commands/pfm/`, and the shipped `templates/project/commands/ptm.md` twin follow in one rename pass.
