...
- B: pfm — `pfm update adopt [--at REF]` pins an existing install's project files to their templates without scaffolding — the bootstrap every pre-scaffold install needs before `check` can report anything; `--at` pins at the blueprint ref the install was last synced from, so the first `check` lists every template change since. `pfm update ignore <template>... [--undo]` keeps a template the project will never take out of `NEW` (counted as `ignored`); `pin --template` un-ignores on adoption. A missing baseline now names both doors (`adopt` for an existing install, `init` for a new one) instead of a bare not-found.

- B: pfm — `/reload` executes without a model turn: a new `UserPromptSubmit` hook (`pfm internal reload-intercept`) runs a human-typed `/reload …` through the reload front in-process and blocks the prompt from ever reaching the model (Claude seats only; a Codex seat still routes through the command body). `pfm chat reload --fresh` reboots into a brand-new session id in the same pane while the old conversation stays resumable from the picker. The `/reload` command card's description is now generated verbatim from `reload.Usage` — the picker shows the exact flags the command accepts, never a hand-maintained restatement. A new global `/handoff` skill writes a structured context file, then reloads fresh with `--then` to continue the task in a clean context window.

- B: pfm — the cosmos seeds a node for every live pane row (quiet chats that never used the chat MCP included) at the row's own last activity; the chronoscope replay stays events-only — the past is the ledger.
...
- C: pfm — Codex `/clear` actually follows the screen. Only a rollout the pane's process holds open may override the status line (the Codex TUI holds none — the resolver's binding-derived identity was circular, so the old row stayed live and the successor showed as RESUMABLE forever); and once Codex titles the successor from its first reply, that title moves the binding FORWARD — a single unclaimed thread the title identifies, in a different lineage, provably newer by its UUIDv7 timestamp — so a human who clears and keeps typing is followed too. Nothing ever moves a binding backward. A pass that moved a binding re-gathers before composing, so the successor renders `●` on the same `pfm ls`.
...
- **For** adopters installed before v0.66.0 — run `pfm update adopt --at <the blueprint ref you last synced from>` once inside the project, then `pfm update check`; review each `UPDATED` diff, `pin` what you applied, `ignore` what you will never take.

- **For** adopters — re-run `pfm install --yes` to wire the new `reload-intercept` hook and link the `/handoff` skill. (cost: one new `UserPromptSubmit` hook on every configured Claude account's `settings.json`)
