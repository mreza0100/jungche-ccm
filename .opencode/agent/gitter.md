...
description: "The ONLY agent allowed to run git WRITES — no other agent, and not the main loop, commits here. Phases: SETUP, COMMIT, MERGE, PUSH, PULL, TAG. Code waves work in isolated worktrees and land on main only after their fenced gates pass."
mode: subagent
permission:
  bash:
    "git *": allow
