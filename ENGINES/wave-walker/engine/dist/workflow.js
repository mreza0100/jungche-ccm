...
class Configs {
  ...
  constructor(rawArgs         ) {
    ...
    if (typeof parsed === 'string') {
      try {
        ...
        throw new Error(
          'wave-walker: args is a string but not valid JSON: ' + ((e         ).message || e),
        );
      }
    }
    const arg = (parsed || {})           ;
    ...
    this.VOTES =
      Number.isInteger(arg.votes) && (arg.votes          ) > 0 ? (arg.votes          ) : 1;
    ...
    this.SCOPE =
      Array.isArray(arg.scope) && (arg.scope             ).length ? (arg.scope            ) : null;
    this.LENSES =
      Array.isArray(arg.lenses) && (arg.lenses             ).length
        ? (arg.lenses            )
        : DEFAULT_LENSES;
  ...
  // absence" discipline as parseInvariants above.
    ...
    const roles = pair(p.roles, 'roles', ['owner', 'elevated'])   
                                           
                 ;
    const fenceLabels = pair(p.fenceLabels, 'fenceLabels', ['org', 'ownership'])   
...
const buildClaimAuditor = ({ rows, repoRoot }                  )         =>
...
const buildClaimExtractor = ({ manifestPath, repoRoot }                    )         =>
...
const buildClaimVerifier = ({ claim: c, question, repoRoot }                   )         =>
...
function runClaimVerifier(
  ...
  return retryAgent           (claimVerifier.buildPrompt({ claim, question, repoRoot }), {
...
function runClaimVerifierBatch(
  ...
  return retryAgent                           (buildClaimVerifierBatch(batch, question, repoRoot), {
...
const buildGateSweep = ({ file, resourceClasses, fenceLabels }               )         =>
...
function computeGateArming(project                            )             {
  ...
  try {
    return {
      ...
      detail: '',
      patterns: {
        resolver: new RegExp(project.gateResolverPattern          ),
        surface: new RegExp(project.gateSurfacePattern          ),
      },
    };
  } catch (e) {
    return {
...
class WaveWalker {
  ...
  async runVerify()                                       {
    ...
    if (solo) {
      ...
      const panel = claims.flatMap((c) =>
        Array.from({ length: CONFIG.VOTES }, (_, v) => ({ c: c           , v })),
      );
    ...
    const taskIds = [
      ...new Set((claims             ).map((c) => c.taskId).filter((t)              => !!t)),
    ];
    return {
      status: 'DONE',
  ...
  async runInvestigate()                                            {
    ...
    const seedLanes         = CONFIG.LENSES.map((lens, i) => ({
      id: 'w0-' + (i + 1),
      kind: 'pursue',
      question: lens,
    }));
  ...
  async runWalk()                                     {
    ...
    const threads               = (scout.threads || []).concat(
      CONFIG.EXTRA_THREADS                ,
    );
    ...
    const authOk =
      ...
      authRuleMustContain.every((tok) => (scout.authRule          ).includes(tok));
    if (!authOk)
      log(
    ...
    const sliceResults = (walked.slice(nT, nT + nJ)                        ).filter(
      (x)                 => !!x,
    );
    const gates                     = (
      walked.slice(nT + nJ, nT + nJ + nG)                           
    )
      .filter((x)                    => !!x)
      .flatMap((s) => (s.gates || []).map((g) => ({ ...g, file: s.file })));
    const securityResults = (
      walked.slice(nT + nJ + nG, nT + nJ + nG + nS)                          
    ).filter((x)                   => !!x);
    const security                        = mergeSecurityResults(
      securityResults,
      nS,
      scout.changedFiles || [],
    );
    ...
    const hunterResults = (
      walked.slice(nT + nJ + nG + nS, nT + nJ + nG + nS + nInv)                                 
    ).filter((x)                          => !!x);
    ...
    const [judgeResults, digestResults, coverageCriticResult] = await Promise.all([
      parallel(
        judgeJobs.map((j) => () => {
          const ctxCards = [
            ...new Set(j.grp.map((x) => x.cardId).filter((id)               => !!id)),
          ]
