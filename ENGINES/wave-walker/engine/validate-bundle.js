...
// 5) PROJECT-LEAK GUARD (permanent) — this is a UNIVERSAL bundle: every project-specific site now arms
//    from the caller's args.project profile (config.ts CONFIG.PROJECT), never from baked-in source. A hit
  ...
  if (hit)
    fail(
      'bundle leaks project-specific term "' +
        term +
        '" — genericize the site in src/ (route it through args.project / CONFIG.PROJECT) and rebuild',
    )
}

console.log(
  '✓ bundle valid — single-file, `export const meta` literal at top, no forbidden globals, no project-specific leak terms',
)
