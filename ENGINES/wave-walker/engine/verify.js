// verify.js — the stale-bundle guard. Rebuilds dist/workflow.js to a disposable temp path (reusing
// build.js's exact `runBuild` pipeline) and byte-diffs the result against the COMMITTED dist/workflow.js.
...
const COMMITTED = join(HERE, 'dist', 'workflow.js');
...
try {
  ...
  if (fresh !== committed)
    fail(
      'dist/workflow.js is STALE — it does not match a fresh rebuild of engine/src/.\n' +
        '  Run `npm run build` in engine/ and commit the regenerated dist/workflow.js alongside your source change.',
    );
  console.log('✓ dist/workflow.js matches a fresh rebuild of engine/src/ — not stale');
} finally {
