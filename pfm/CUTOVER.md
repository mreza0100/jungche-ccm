# pfm (Professor-Fleet-Manager) cutover and rollback

The cutover is EXECUTED: `~/.zshrc` sources `pfm/shim/pfm.zsh`, the
hide scripts are `exec pfm hide --self` delegates, and the legacy
...
`/tmp/pfm-legacy-2026-08-11.tar.gz`). MCP registration is deliberately
...
engine lives at `~/.professor/pfm/` and the satellite bundle at
`~/.professor/blueprint/templates/host-swap/`; read every
`oldbox/scripts/` reference below as that bundle directory.

## Preconditions

1. `pfm ls --check` has reported an empty or allowlisted-only diff at
...
From `~/work/host-ops/pfm`:

```sh
CGO_ENABLED=0 GOTOOLCHAIN=local GOTELEMETRY=off \
  mise exec -- go build -trimpath -o /tmp/pfm.wp11 ./cmd/pfm
file /tmp/pfm.wp11
install -d -m 0755 "$HOME/.local/bin"
install -m 0755 /tmp/pfm.wp11 "$HOME/.local/bin/pfm"
...
```sh
"$HOME/.local/bin/pfm" legacy import
"$HOME/.local/bin/pfm" doctor
```
...
```zsh
[[ -r "$HOME/work/host-ops/oldbox/scripts/pfm.zsh" ]] && source "$HOME/work/host-ops/oldbox/scripts/pfm.zsh"
```

with:

```zsh
[[ -r "$HOME/work/host-ops/pfm/shim/pfm.zsh" ]] && source "$HOME/work/host-ops/pfm/shim/pfm.zsh"
```

Validate before reloading:

```sh
zsh -n "$HOME/work/host-ops/pfm/shim/pfm.zsh"
zsh -n "$HOME/.zshrc"
```
...
```sh
#!/usr/bin/env bash
exec "$HOME/.local/bin/pfm" hide --self "$@"
```
...
WP12 implements and tests the stdio-only `pfm mcp` server. Do not
...
Update `oldbox/docs/pfm.md` with these changes:

- State that `~/.zshrc` sources `pfm/shim/pfm.zsh` and that the
  installed binary is `~/.local/bin/pfm`.
...
- Keep the shell surface (`cc`, `cc1`, `cc2`, `cx`, `cc-swap`,
...
- Warm `pfm ls --plain` completes in under one second wall time.
- `pfm ls --check` is empty or allowlisted-only.
- TUI Ctrl-T, Ctrl-R, and Ctrl-X round trips work.
- `/bb` in a scratch chat hides and exits it; a later real prompt
  auto-unhides it.
- `pfm resolve session <name>` output is identical to `chat.sh`
  `_resolve_session`.
...
```sh
"$HOME/.local/bin/pfm" legacy export
```
