...
const (
	checkAllowlistEnv  = "PFM_CHECK_ALLOWLIST"
...
func runLSCheck(ctx context.Context, stdout, stderr io.Writer) int {
	...
	if err != nil {
		fmt.Fprintf(stderr, "pfm ls --check: %v\n", err)
		return 1
	}
	...
	if err != nil {
		fmt.Fprintf(stderr, "pfm ls --check: verify Codex candidate bound: %v\n", err)
		return 1
	}
	...
	if err != nil {
		fmt.Fprintf(stderr, "pfm ls --check: verify Codex source schema: %v\n", err)
		return 1
	}
	codexRolloutFiles, err := fleetcheck.CodexRolloutFileIDs(resolved.CodexRoot)
	if err != nil {
		fmt.Fprintf(stderr, "pfm ls --check: verify Codex rollout files: %v\n", err)
		return 1
	}
	...
	if err != nil {
		fmt.Fprintf(stderr, "pfm ls --check: verify legacy name cache: %v\n", err)
		return 1
	}
	claudeTranscripts, err := fleetcheck.ClaudeTranscriptIDs(resolved.ClaudeRoots)
	if err != nil {
		fmt.Fprintf(stderr, "pfm ls --check: verify Claude store: %v\n", err)
		return 1
	}
	...
	if err != nil {
		fmt.Fprintf(
			stderr,
			"pfm ls --check: verify Codex machine-spawned threads: %v\n",
			err,
		)
		return 1
	}
	...
	if err != nil {
		fmt.Fprintf(stderr, "pfm ls --check: verify Codex lineages: %v\n", err)
		return 1
	}
	...
	if unallowed != 0 {
		fmt.Fprintf(
			stdout,
			"pfm check: %d difference(s), %d allowlisted\n",
			unallowed,
			allowed,
		)
		return 1
	...
	fmt.Fprintf(
		stdout,
		"pfm check: parity (%d tuples, %d allowlisted deviation(s))\n",
		len(own),
		allowed,
	)
	return 0
...
// beside the legacy oracle the checker already resolved. The oracle ships in
// two layouts: flat bundle clones keep pfm/ under its directory, while this
// repository keeps pfm/ beside blueprint/. The installed binary runs from
// ~/.local/bin with any working directory, so both oracle-relative layouts
// are required.
func resolveCheckAllowlist(executable, working, home string) (string, error) {
	...
	if home != "" {
		if legacyScript, err := fleetcheck.ResolveLegacyScript(home); err == nil {
			oracleDir := filepath.Dir(legacyScript)
			candidates = append(
				candidates,
				filepath.Join(
					oracleDir,
					"pfm", "testdata", checkAllowlistFile,
				),
