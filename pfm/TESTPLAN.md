...
| configured engine self-doctors | JAIL | supported Claude/Codex doctor commands run under a 5s context; unsupported or interactive-only surfaces say unavailable; failures remain broken | `internal/deps/probe_test.go`, `doctor_external_test.go` |
...
| Limits reads Codex auth and live usage through a 20s uncached transport; stale statusline cache files are ignored and auth/HTTP/decode failures stay visible         | JAIL      | `stats/limits.go`, `stats/limits_test.go`, `cmd/pfm/limits_accounts_test.go`      | defect RED |
...
| install dependency preflight uses the doctor registry; missing required deps refuse before installer writes and `--force` cannot bypass                                                 | JAIL             | `doctor_external_test.go`, `internal/deps/probe_test.go`                                  |            |
