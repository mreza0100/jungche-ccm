...
28c. ✅ DONE — Ctrl+S composer-stash semantics against a REAL claude session
(`internal/inject/claude_stash_real_test.go`, `TestRealClaudeStashSemantics`, skipped unless
`PFM_REAL_CLAUDE=1`; v2.1.257, 2026-09-03): typed draft + C-s empties the composer; C-s on an
empty composer pops it back; a `/help` command submit AND (`PFM_REAL_CLAUDE_TURN=1`) a real
message submit both restore the stashed draft (the latter also prints an explicit "Draft
restored" UI hint, seen only on that async-turn-boundary restore, not on the synchronous pops);
stashing a second draft while one is already parked OVERWRITES it — last-write-wins, one slot,
