...
CC_DB="$CC_FLEET_HOME/cc-db.sh"
cc-ls() {
  local -a rows=()
  local prompts
  prompts="$(bash "$CC_DB")" || {
    echo "legacy fixture: database seam rejected" >&2
    return 64
  }
  rows+=($'project\t'"$prompts"$'\t0\tR\tchat-7\t/work/project\t\e[2m↻\e[0m project        │ name                           │     7p │   12B │ now')
...
const legacyFixtureDatabase = `#!/usr/bin/env bash
set -u
[[ -n "${PFM_DB:-}" && "$PFM_DB" == "${CC_FLEET_DB:-}" ]] || {
  echo "legacy fixture: PFM_DB and CC_FLEET_DB do not name one shadow store" >&2
  exit 64
}
[[ -n "${CC_CHATCACHE:-}" && "$CC_CHATCACHE" != /tmp/cc-sid/* ]] || {
  echo "legacy fixture: CC_CHATCACHE can reach the live fallback" >&2
  exit 65
}
printf '%s' 7
`
...
func writeLegacyFixture(t *testing.T, bundle string) string {
	...
	if err := os.WriteFile(
		filepath.Join(bundle, "cc-db.sh"),
		[]byte(legacyFixtureDatabase),
		0o700,
	); err != nil {
		t.Fatal(err)
	}
...
func TestResolveLegacyScriptReadsTheZshrcSourceLine(t *testing.T) {
	...
	bundle := filepath.Join(root, "clone")
	scriptPath := writeLegacyFixture(t, bundle)
	shim := filepath.Join(bundle, "pfm", "shim", activeShimName)
	if err := os.MkdirAll(filepath.Dir(shim), 0o700); err != nil {
		t.Fatal(err)
	}
	if err := os.WriteFile(shim, []byte("# sourced wrapper\n"), 0o600); err != nil {
		t.Fatal(err)
	}
...
func TestShadowLegacyScriptNormalizesFrozenFindMetaParsing(t *testing.T) {
	const stale = `mt="${line%%.*}";`
	const normalized = `mt="${line%%$'\t'*}";`
	source := stale + "\n" + stale + "\n"
	shadow := shadowLegacyScript(source, "/jailed/sid", "/jailed/claude", "/jailed/codex")
	if strings.Contains(shadow, stale) {
		t.Fatalf("shadow retained the frozen oracle's stale dot split: %s", shadow)
	}
	if count := strings.Count(shadow, normalized); count != 2 {
		t.Fatalf("shadow normalized %d find-meta parses, want 2: %s", count, shadow)
	}
}
...
func TestZshrcLegacyScriptTranslatesTheShimToItsBundleOracle(t *testing.T) {
	...
	line := `[[ -r "` + home + `/bundle/pfm/shim/pfm.zsh" ]] && source "` + home + `/bundle/pfm/shim/pfm.zsh"`
