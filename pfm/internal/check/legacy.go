...
const (
	...
	LegacyOutputEnv = "PFM_CHECK_LEGACY_OUTPUT"
	// LegacyScriptEnv overrides the read-only reference script location.
	LegacyScriptEnv = "PFM_LEGACY_ZSH"

	// LegacyScriptName is the picker this checker shadow-runs.
	LegacyScriptName = "cc-fleet.zsh"
	// activeShimName is the sourced wrapper whose location reveals the oracle.
	activeShimName = "pfm.zsh"
...
//
//	[[ -r "/path/to/pfm.zsh" ]] && source "/path/to/pfm.zsh"
var legacySourceLinePattern = regexp.MustCompile(
	`source[ \t]+"?([^"\n]*(?:` + regexp.QuoteMeta(activeShimName) + `|` +
...
// ResolveLegacyScript locates the legacy cc-fleet.zsh oracle whose behaviour this
...
//
//  1. $PFM_LEGACY_ZSH — the explicit answer. Taken as given; an unreadable
...
//     PROGRAM and lives at the repo root, so from <repo>/pfm the script is
...
//     directory. pfm.dev is built into the tree root, so the first form
...
func ResolveLegacyScript(home string) (string, error) {
	...
	return "", fmt.Errorf(
		"locate the live %s: set %s, or keep it beside the pfm tree "+
			"(tried %s)",
		LegacyScriptName,
		LegacyScriptEnv,
...
func zshrcLegacyScript(zshrc string) string {
	...
	for _, line := range strings.Split(string(content), "\n") {
		...
		sourced := match[1]
		// Post-cutover the shell sources <bundle>/pfm/shim/pfm.zsh.
...
func runLegacy(ctx context.Context, resolved paths.Values) (string, error) {
	...
	root, err := os.MkdirTemp("", "pfm-check-")
	if err != nil {
...
func shadowLegacyScript(script, sidDir, claudeRoot, codexRoot string) string {
	// The frozen oracle predates cc_find_meta's normalized integer-mtime
	// contract and still strips a GNU fractional suffix at the first dot. With
	// an integer field, that dot belongs to the transcript path and the entire
	// tab-bearing tail reaches zsh arithmetic. Adapt only the disposable shadow;
	// the byte-frozen oracle remains the parity reference on disk.
...
func shadowEnvironment(
	...
	shadowHome, shadowSID, runtimeDir, binDir, shadowDatabase string,
) []string {
	...
	values := []string{
		...
		// The frozen oracle still carries the old spelling, while the renamed
		// cc-db.sh sibling reads the new one. Keep both on the same snapshot or
		// the parity run silently compares stores from different namespaces.
		"CC_FLEET_DB=" + shadowDatabase,
		"PFM_DB=" + shadowDatabase,
		// If sqlite3 is unavailable or the snapshot cannot be opened, cc-db.sh
		// falls back to its name-cache file. Jail that fallback too: its default
		// is the live /tmp/cc-sid/.namecache.v5.
		"CC_CHATCACHE=" + filepath.Join(shadowSID, ".namecache.v5"),
		"PATH=" + strings.Join(
