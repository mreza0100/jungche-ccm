...
This document describes PNG (Portable Network Graphics), an extensible file format for the [lossless](#dfn-lossless), portable,
    well-compressed storage of static and animated raster images. PNG provides a patent-free replacement for GIF and can also
    replace many common uses of TIFF. [Indexed-color](#3indexedColour), [greyscale](#dfn-greyscale), and [truecolor](#3truecolour) images are supported, plus an optional alpha
...
W3C recommends the wide deployment of this specification as a standard for the Web.
        ...
        [W3C Patent
          Policy](https://www.w3.org/policies/patent-policy/).
                ...
                W3C maintains a
                [public list of any patent disclosures](https://www.w3.org/groups/wg/png/ipr)
...
- Completeness: it should be possible to represent [truecolor](#3truecolour),[indexed-color](#3indexedColour), and[greyscale](#dfn-greyscale)images, in each case with the option of transparency, color space information, and ancillary information such as textual comments.
...
This specification specifies a datastream and an associated file format, Portable Network Graphics (PNG, pronounced "ping"),
    for a [lossless](#dfn-lossless), portable, compressed individual computer graphics image or frame-based animation, transmitted across the
    Internet.
...
- chromaticity
- pair of *x*and*y*values in the*xyY*space specified at [[COLORIMETRY](#bib-colorimetry)].NoteChromaticity is a measure of the quality of a color regardless of its luminance.
- composite (verb)
- form an image by merging a foreground image and a background image, using transparency information to determine
      ...
      NoteThe foreground image is said to be [composited](#dfn-composited)against the background.
- datastream
-
        sequence of [bytes](#dfn-byte).
- deflate
-
        member of the [LZ77](#3LZ77)family of compression methods.SOURCE: [ [RFC1951](#bib-rfc1951)]
- frame
...
-
        ...
        NoteSoftware causes an image to appear on screen by loading the image into the [frame buffer](#dfn-frame-buffer).
...
-
        ...
        NoteLuminance and [chromaticity](#dfn-chromaticity)together fully define a measured color. For a formal definition, see [[COLORIMETRY](#bib-colorimetry)].
- LZ77
...
-
        [transfer function](#dfn-transfer-function)defined in [[ITU-R-BT.2100](#bib-itu-r-bt.2100)] Table 4. (An absolute display-referred system.)NoteOnly RGB may be used in PNG, ICtCp is NOT supported.
- PNG decoder
-
...
-
        a four-byte unsigned integer limited to the range 0 to 2 31-1.NoteThe restriction is imposed in order to accommodate languages that have difficulty with unsigned four-byte values.
...
-
        an image format capable of storing images with a relatively low dynamic range of 5-8 [stops](#dfn-stop). Examples include [[SRGB](#bib-srgb)], [[Display-P3](#bib-display-p3)], [[ITU-R-BT.709](#bib-itu-r-bt.709)].NoteStandard dynamic range is independent of the primaries and hence, gamut. Wide color gamut [SDR](#dfn-sdr)formats are supported by PNG.
- stop
...
- zlib
-
        [deflate](#dfn-deflate)-style compression method.SOURCE: [ [rfc1950](#bib-rfc1950)]NoteAlso refers to the name of a library containing a sample implementation of this method.
- Cyclic Redundancy Code
- CRC
-
        type of check value designed to detect most transmission errors. NoteA decoder calculates the CRC for the received data and checks by comparing it to the CRC calculated by the encoder and appended to the data. A mismatch indicates that the data or the CRC were corrupted in transit.
- Cathode Ray Tube
- CRT
...
Some PNG images — called Animated PNG (APNG) — also
      ...
      the [static image](#dfn-static-image). Non-animation-capable displays (such as printers) will display the [static image](#dfn-static-image) rather than
      the animation sequence.

The [static image](#dfn-static-image), and each individual frame of an [animated image](#dfn-animated-image), corresponds to a *reference image*
...
This specification specifies the PNG datastream, and places some requirements on PNG encoders, which generate PNG
      datastreams, PNG decoders, which interpret PNG datastreams, and [PNG editors](#dfn-png-editor), which transform one PNG datastream into
...
- by specifying a [gamma value](#dfn-gamma-value)and the 1931 CIE*x,y*chromaticities of the red, green, and blue primaries used in the image and the reference[white point](#dfn-white-point).
...
For high-end applications the first two methods provides the most flexibility and control. The third method enables one
      ...
      were widely adopted, enables the exact chromaticities of the RGB data to be specified, along with the [gamma](#dfn-gamma) correction
      to be applied (see [C. Gamma and chromaticity](#C-GammaAppendix)). However, color-aware applications will prefer one of the first three
      methods, while color-unaware applications will typically ignore all four methods.

[Table 1](#color-chunk-precendence) is a list of chunk types that provide color space information,
...
[Gamma](#dfn-gamma) correction is not applied to the alpha channel, if present. Alpha samples are always full-range and represent
      a linear fraction of full opacity.

Mastering metadata may also be provided.

A number of transformations are applied to the reference image to create the PNG image to be encoded (see [Figure 3](#reference-to-png-transformation)). The transformations are applied in the following sequence, where square brackets
...
```
When every pixel is either fully transparent or fully opaque, the [alpha separation](#3alphaSeparation), [alpha compaction](#3alphaCompaction), and
        [indexing](#3indexing) transformations can cause the recovered reference image to have an alpha sample depth different from the
...
If the number of distinct pixel values is 256 or less, and the RGB sample depths are not greater than 8, and the alpha
        ...
        [indexed-color](#3indexedColour) representation, achieved through an indexing transformation, may be more efficient for encoding.
...
A suggested palette or palettes may be constructed even when the PNG image is not [indexed-color](#3indexedColour) in order to assist
        viewers that are capable of displaying only a limited number of colors.

For [indexed-color](#3indexedColour) images, encoders can rearrange the palette so that the table entries with the maximum alpha value are
...
In the PNG image, not all sample depths are supported (see [6.1 Color types and values](#6Colour-values)), and all channels shall have
        ...
        next allowable range for the PNG image. [Figure 5](#scaling-sample-values) shows how samples of depth 3 might be mapped
...
Allowing only a few sample depths reduces the number of cases that decoders have to cope with. [Sample depth
        scaling](#3sampleDepthScaling) is reversible with no loss of data, because the reference image sample depths can be recorded in the PNG
        datastream. In the absence of recorded sample depths, the reference image sample depth equals the PNG image sample depth.
        See [12.4 Sample depth scaling](#12Sample-depth-scaling) and [13.12 Sample depth rescaling](#13Sample-depth-rescaling).
...
The format of each pixel depends on the PNG image type and the bit depth. For PNG image types other than indexed-color,
      the bit depth specifies the number of bits per sample, not the total number of bits per pixel. For [indexed-color](#3indexedColour) images, the
...
- [Truecolor with alpha](#dfn-truecolor-with-alpha): red, green, blue, alpha.
- [Greyscale with alpha](#dfn-greyscale-with-alpha): grey, alpha.
- [Truecolor](#3truecolour): red, green, blue.
- [Greyscale](#dfn-greyscale): grey.
- [Indexed-color](#3indexedColour): palette index.

A conceptual model of the process of encoding a PNG image is given in [Figure 7](#encoding-png-image). The steps
        refer to the operations on the array of pixels or indices in the PNG image. The [palette](#3palette) and [alpha table](#3alphaTable) are not encoded in
        this way.
...
Pass extraction (see [Figure 7](#encoding-png-image)) splits a PNG image into a sequence of reduced images where the
        ...
        passes for a sample image are illustrated in [Figure 7](#encoding-png-image). See [8. Interlacing and pass extraction](#8Interlace).

Each row of pixels, called a scanline, is represented as a sequence of bytes.

PNG allows [image data](#dfn-image-data) to be filtered before it is compressed. Filtering can improve the compressibility of the
        data. The filter operation is deterministic, reversible, and lossless. This allows the decompressed data to be
        reverse-filtered in order to obtain the original data. See [7.3 Filtering](#7Filtering).

The sequence of filtered scanlines in the pass or passes of the PNG image is compressed (see [Figure 9](#compression)) by one of the defined compression methods. The concatenated filtered scanlines form the input to the
        compression stage. The output from the compression stage is a single compressed datastream. See [10. Compression](#10Compression).
...
Ancillary information may be associated with an image. Decoders may ignore all or some of the ancillary information. The
      types of ancillary information provided are described in [Table 2](#table41).

| Type of information | Description |
|---|---|
| Animation information | An animated image, defined as a series of frames with associated timing, position and handling information, to be
            displayed if the viewer is capable of doing so. For other cases such as printers, the [static image](#dfn-static-image)will be
            displayed instead. |
| Background color | Solid background color to be used when presenting the image if no better option is available. |
| Coding-independent code points | Identifies the color space by enumerating metadata such as the [transfer function](#dfn-transfer-function)and color primaries.
            Originally for[SDR](#dfn-sdr)and[HDR](#dfn-hdr)video, also used for still and animated images. |
...
| Gamma and chromaticity | [Gamma value](#dfn-gamma-value)of the image with respect to the desired output intensity, and[chromaticity](#dfn-chromaticity)characteristics
...
The PNG datastream consists of a PNG signature (see [5.2 PNG signature](#5PNG-file-signature)) followed by a sequence of
        chunks (see [11. Chunk specifications](#11Chunks)). Each chunk has a chunk type which specifies its function.

Chunk types are four-byte sequences chosen so that they correspond to readable labels when interpreted in the ISO
        646.IRV:1991 [[ISO646](#bib-iso646)] character set. The first four are termed critical chunks, which shall be understood and correctly
        interpreted according to the provisions of this specification. These are:

-
            [IHDR](#11IHDR): image header, which is the first chunk in a PNG datastream.
-
            [PLTE](#11PLTE): palette table associated with indexed PNG images.
-
            [IDAT](#11IDAT): image data chunks.
-
            [IEND](#11IEND): image trailer, which is the last chunk in a PNG datastream.

The remaining chunk types are termed ancillary chunk types, which encoders may generate and decoders may interpret.

- Transparency information: [tRNS](#11tRNS)(see[11.3.1 Transparency information](#11transinfo)).
- Color space information: [cHRM](#11cHRM),[gAMA](#11gAMA),[iCCP](#11iCCP),[sBIT](#11sBIT),[sRGB](#11sRGB),[cICP](#cICP-chunk),[mDCV](#mDCV-chunk)(see[11.3.2 Color space information](#11addnlcolinfo)).
- Textual information: [iTXt](#11iTXt),[tEXt](#11tEXt),[zTXt](#11zTXt)(see[11.3.3 Textual information](#11textinfo)).
- Miscellaneous information: [bKGD](#11bKGD),[hIST](#11hIST),[pHYs](#11pHYs),[sPLT](#11sPLT),[eXIf](#eXIf)(see[11.3.4 Miscellaneous information](#11addnlsiinfo)).
- Time information: [tIME](#11tIME)(see[11.3.5 Time stamp information](#11timestampinfo)).
- Animation information: [acTL](#acTL-chunk),[fcTL](#fcTL-chunk),[fdAT](#fdAT-chunk)(see[11.3.6 Animation information](#animation-information)).

Animated PNG (APNG) extends the original, static-only PNG format,
        adding support for [frame](#dfn-frame)-based animated images. It is intended to
        be a replacement for simple animated images that have traditionally used the GIF format [[GIF](#bib-gif)], while adding support for
        24-bit images and 8-bit transparency, which GIF lacks.

APNG is backwards-compatible with earlier versions of PNG; a non-animated PNG decoder will ignore the ancillary
        APNG-specific chunks and display the [static image](#dfn-static-image).
...
To be recognized as an APNG, an [acTL](#acTL-chunk) chunk must appear in the stream before any
        [IDAT](#11IDAT) chunks. The [acTL](#acTL-chunk) structure is [described below](#animation-information).
...
Conceptually, at the beginning of each play the [output buffer](#dfn-output-buffer) shall be completely initialized to a [fully
        transparent black](#dfn-fully-transparent-black) rectangle, with width and height dimensions from the [IHDR](#11IHDR)
        chunk.

The static image may be included as the first frame of the animation by the presence of a single [fcTL](#fcTL-chunk) chunk before [IDAT](#11IDAT). Otherwise, the static image is not part of
        the animation.

Subsequent frames are encoded in [fdAT](#fdAT-chunk) chunks, which have the same structure as
        [IDAT](#11IDAT) chunks, except preceded by a [sequence
...
The boundaries of the entire animation are specified by the width and height parameters of the [IHDR](#11IHDR) chunk, regardless of whether the default image is part of the animation. The default image should be
        appropriately padded with [fully transparent black](#dfn-fully-transparent-black) pixels if extra space will be needed for later frames.

Each frame is identical for each play, therefore it is safe for applications to cache the frames.

The [fcTL](#fcTL-chunk) and [fdAT](#fdAT-chunk) chunks have a
...
The first [fcTL](#fcTL-chunk) chunk shall contain sequence number 0, and the sequence numbers
        in the remaining [fcTL](#fcTL-chunk) and [fdAT](#fdAT-chunk) chunks shall
...
The output buffer is a pixel array with dimensions specified by the width and height parameters of the PNG [IHDR](#11IHDR) chunk. Conceptually, each frame is constructed in the output buffer before being
        [composited](#dfn-composited) onto the [canvas](#dfn-canvas). The contents of the output buffer are available to the decoder. The corners of
        the output buffer are mapped to the corners of the [canvas](#dfn-canvas).

The canvas is the area on the output device on which the frames are to be displayed. The contents of the
        canvas are not necessarily available to the decoder. If a [bKGD](#11bKGD) chunk exists, it may be
...
PNG decoders should detect errors as early as possible, recover from errors whenever possible, and fail gracefully
      otherwise. The error handling philosophy is described in detail in [13.1 Error handling](#13Decoders.Errors).
...
The PNG datastream consists of a PNG signature followed by a sequence of chunks. It
      is the result of encoding a [PNG image](#3PNGimage).
...
This signature indicates that the remainder of the datastream contains a single PNG image, consisting of a series of
      chunks beginning with an [IHDR](#11IHDR) chunk and ending with an [IEND](#11IEND) chunk.

This signature differentiates a PNG datastream from other types of [datastream](#dfn-datastream) and allows early detection of some
      transmission errors.

Each chunk consists of three or four fields (see [Figure 10](#figure411)). The meaning of the fields is described in
      [Table 5](#table51). The chunk data field may be empty.

| Name | Description |
|---|---|
| Length | A [PNG four-byte unsigned integer](#dfn-png-four-byte-unsigned-integer)giving the number of bytes in the chunk's data field. The length countsonlythe data field,notitself, the chunk type, or the CRC. Zero is a valid
            length. Although encoders and decoders should treat the length as unsigned, its value shall not exceed 231-1
            bytes. |
| Chunk Type | A sequence of four bytes defining the chunk type. Each byte of a chunk type is restricted to the hexadecimal values 41
            to 5A and 61 to 7A. These correspond to the uppercase and lowercase ISO 646 [ [ISO646](#bib-iso646)] letters
            ...
            strings. For example, it would not be correct to represent the chunk type[IDAT](#11IDAT)by
            the equivalents of those letters in the UCS 2 character set. Additional naming conventions for chunk types are
            discussed in[5.4 Chunk naming conventions](#5Chunk-naming-conventions). |
| Chunk Data | The data bytes appropriate to the chunk type, if any. This field can be of zero length. |
| CRC | A four-byte CRC calculated on the preceding bytes in the chunk, including the chunk type field and chunk data
            fields, but notincluding the length field. The CRC can be used to check for corruption of the
            data. The CRC is always present, even for chunks containing no data. See[5.5 CRC algorithm](#5CRC-algorithm). |
...
Chunk types are chosen to be meaningful names when the bytes of the chunk type are interpreted as ISO 646 letters
      [[ISO646](#bib-iso646)]. Chunk types are assigned so that a decoder can determine some properties of a chunk even when the type is not
...
The naming rules are normally of interest only when the decoder does not recognize the chunk's type, as specified at
      [13. PNG decoders and viewers](#13Decoders).
...
|---|---|---|
| Ancillary bit: first byte | 0 (uppercase) = critical, 1 (lowercase) = ancillary. | Critical chunks are necessary for successful display of the contents of the datastream, for example the image header
            chunk ( [IHDR](#11IHDR)). A decoder trying to extract the image, upon encountering an unknown
            chunk type in which the ancillary bit is 0, shall indicate to the user that the image contains information it cannot
            safely interpret.Ancillary chunks are not strictly necessary in order to meaningfully display the contents of the datastream, for example the time chunk ( [tIME](#11tIME)). A decoder encountering an unknown chunk type in
            which the ancillary bit is 1 can safely ignore the chunk and proceed to display the image. |
| Private bit: second byte | 0 (uppercase) = public, 1 (lowercase) = private. | Public chunks are reserved for definition by the W3C. The definition of private chunks is specified at [12.10.1 Use of private chunks](#12Use-of-private-chunks). The names of private chunks have a lowercase second letter, while the names of public
...
| Safe-to-copy bit: fourth byte | 0 (uppercase) = unsafe to copy, 1 (lowercase) = safe to copy. | This property bit is not of interest to pure decoders, but it is needed by [PNG editors](#dfn-png-editor). This bit defines the
            proper handling of unrecognized chunks in a datastream that is being modified. Rules for[PNG editors](#dfn-png-editor)are
            discussed further in[14.2 Behavior of PNG editors](#14Ordering). |
...
CRC fields are calculated using standardized CRC methods with pre and post conditioning, as defined by [[ISO-3309](#bib-iso-3309)] and [[ITU-T-V.42](#bib-itu-t-v.42)]. The CRC polynomial employed— which is identical to that used in the GZIP file format
      specification [[RFC1952](#bib-rfc1952)]— is
...
The constraints on the positioning of the individual chunks are listed in [Table 7](#table53) and illustrated
      diagrammatically for static images in [Figure 11](#lattice-diagram-with-plte) and [Figure 12](#lattice-diagram-without-plte), for animated images where the static image forms the first frame in [Figure 13](#lattice-apng-static-with-plte) and [Figure 14](#lattice-apng-static-without-plte), and for animated images where the
      static image is not part of the animation in [Figure 15](#lattice-apng-nostatic-with-plte) and [Figure 16](#lattice-apng-nostatic-without-plte). These lattice diagrams represent the constraints on positioning imposed by this
      ...
      connected. The superscript associated with the chunk type is defined in [Table 8](#table54). It indicates whether the
...
A private chunk *SHOULD NOT* be defined merely to carry textual information of interest to a human user. Instead [iTXt](#11iTXt) chunk *SHOULD* BE used and corresponding keyword *SHOULD* BE used and a suitable keyword
        defined.

Listing private chunks at [[PNG-EXTENSIONS](#bib-png-extensions)] reduces, but does not eliminate, the chance that the same private chunk is
...
[Private field values](#dfn-private-field-values) *SHOULD NOT* appear in publicly available software or datastreams since they can result in
      datastreams that are unreadable by PNG decoders as detailed at [13. PNG decoders and viewers](#13Decoders).

As explained in [4.5 PNG image](#4Concepts.PNGImage) there are five types of PNG

       image. Corresponding to each type is a [color type](#3colourType), which is the sum of the following values: 1
      (palette used), 2 ([truecolor](#3truecolour) used) and 4 (alpha used). [greyscale](#dfn-greyscale) and [truecolor](#3truecolour) images may have an explicit
      alpha channel. The PNG image types and corresponding [color types](#3colourType) are listed in [Table 9](#table6.1).
...
Greyscale samples represent luminance if the transfer curve is indicated (by [gAMA](#11gAMA),
      [sRGB](#11sRGB), [iCCP](#11iCCP)) or [cICP](#cICP-chunk); or device-dependent greyscale if not.
      RGB samples represent calibrated color information if the color space is indicated (by [gAMA](#11gAMA) and [cHRM](#11cHRM), [sRGB](#11sRGB),  [iCCP](#11iCCP),  or [cICP](#cICP-chunk); or uncalibrated device-dependent color if not.

Sample values are not necessarily proportional to light intensity; the [gAMA](#11gAMA) chunk
      ...
      properly. See [4.3 Color spaces](#4Concepts.ColourSpaces), [13.13 Decoder gamma handling](#13Decoder-gamma-handling) and [C. Gamma and chromaticity](#C-GammaAppendix).
...
-
          [Truecolor with alpha](#dfn-truecolor-with-alpha),[greyscale with alpha](#dfn-greyscale-with-alpha): an alpha channel is part of the image array.
-
          [truecolor](#3truecolour),[greyscale](#dfn-greyscale): A[tRNS](#11tRNS)chunk contains a single pixel value distinguishing the fully transparent pixels from the fully opaque pixels.
- [Indexed-color](#3indexedColour): A- [tRNS](#11tRNS)chunk contains the alpha table that associates an alpha sample with each palette entry.
-
          [truecolor](#3truecolour),[greyscale](#dfn-greyscale),[indexed-color](#3indexedColour): there is no[tRNS](#11tRNS)chunk present and all pixels are fully opaque.
...
An alpha channel included in the image array has 8-bit or 16-bit samples, the same size as the other samples. The alpha
      ...
      indicate partially transparent pixels that can be [composited](#dfn-composited) against a background image to yield the delivered
      image.
...
The color values in a pixel are not premultiplied by the alpha value assigned to the pixel. This rule is sometimes called
      ...
      value; in effect, such an image is already [composited](#dfn-composited) against a black background. PNG does **not** use
      premultiplied alpha. In consequence an image editor can take a PNG image and easily change its transparency.) See [12.3 Alpha channel creation](#12Alpha-channel-creation) and [13.16 Alpha channel processing](#13Alpha-channel-processing).

All integers that require more than one byte shall be in [network byte order](#dfn-network-byte-order) (as illustrated in [Figure 17](#integer-representation-in-png) ): the most significant byte comes first, then the less significant bytes in descending
...
[PNG four-byte unsigned integers](#dfn-png-four-byte-unsigned-integer) are limited to the range 0 to 231-1 to accommodate languages that have
      difficulty with unsigned four-byte values.

A PNG image (or pass, see [8. Interlacing and pass extraction](#8Interlace)) is a rectangular pixel array, with pixels appearing left-to-right
...
Pixels within a scanline are always packed into a sequence of bytes with no wasted bits between pixels. Scanlines always
      begin on byte boundaries. Permitted bit depths and [color types](#3colourType) are restricted so that in all cases the packing is
      simple and efficient.

In PNG images of [color type](#3colourType) 0 (greyscale) each pixel is a single sample, which may have precision less than a byte
...
In PNG images of [color type](#3colourType) 3 (indexed-color) each pixel is a single palette index. These indices are packed into
      bytes in the same way as the samples for [color type](#3colourType) 0.
...
PNG images that are not [indexed-color](#3indexedColour) images may have sample values with a bit depth of 16. Such sample values are in
      [network byte order](#dfn-network-byte-order) (MSB first, LSB second). PNG permits multi-sample pixels only with 8 and 16-bit samples, so multiple
      samples of a single pixel are never packed into one byte.

A filter method is a transformation applied to an array of [scanlines](#dfn-scanline) with the aim of
      improving their compressibility.

PNG standardizes one [filter method](#3filter) and several filter types that may be used to prepare [image data](#dfn-image-data) for
      compression. It transforms the byte sequence into an equal length sequence of bytes preceded by a filter type byte (see
      [Figure 18](#serializing-and-filtering-scanline) for an example).

The encoder shall use only a single [filter method](#3filter) for an interlaced PNG image, but may use different filter types
...
The filter type byte is not considered part of the [image data](#dfn-image-data), but it is included in the datastream sent to the
      compression step. See [9. Filtering](#9Filters).

Pass extraction (see [Figure 4.8](#figure48)) splits a PNG image into a sequence of reduced images (the
...
7 7 7 7 7 7 7 7
```
[Figure 4.8](#figure48) shows the seven passes of interlace method 1. Within each pass, the selected pixels are
      ...
      will be a reduced image of two scanlines, each containing four pixels (see [Figure 4.8](#figure48)).
...
PNG allows for a number of [filter methods](#3filter). All the reduced images in an interlaced image shall use a single
      [filter method](#3filter). Only [filter method](#3filter) 0 is defined by this specification. Other [filter methods](#3filter) are reserved
      for future standardization. [Filter method](#3filter) 0 provides a set of five filter types, and individual scanlines in each
...
PNG imposes no additional restriction on which filter types can be applied to an interlaced PNG image. However, the filter
      types are not equally effective on all types of data. See [12.7 Filter selection](#12Filter-selection).
...
Filtering transforms the byte sequence in a scanline to an equal length sequence of bytes preceded by the filter type.
      ...
      [13.10 Interlacing and progressive display](#13Progressive-display).

Filters are applied to **bytes**, not to pixels, regardless of the bit depth or [color type](#3colourType) of the
      image. The filters operate on the byte sequence formed by a scanline that has been represented as described in [7.2 Scanlines](#7Scanline). If the image includes an alpha channel, the alpha data is filtered in the same way as the [image
      data](#dfn-image-data).
...
[Figure 19](#filter-byte-positions) shows the relative positions of the bytes `x`, `a`, `b`,
      and `c`.

[Filter method](#3filter) 0 defines five basic filter types as listed in [Table 11](#9-table91). `Orig(y)`
      ...
      Paeth filter type `PaethPredictor` [[Paeth](#bib-paeth)] is defined below.

[Filter method](#3filter) 0 specifies exactly this set of five filter types and this shall not be extended. This ensures that
      ...
      the [filter method](#3filter) in [11.2.1 IHDR Image header](#11IHDR).
...
The Paeth filter type computes a simple linear function of the three neighboring pixels (left, above, upper left), then
      ...
      adaptation of the technique due to Alan W. Paeth [[Paeth](#bib-paeth)].

The PaethPredictor function is defined in the code below. The logic of the function and the locations of the bytes
      `a`, `b`, `c`, and `x` are shown in [Figure 20](#paethpredictor-function).
...
Only PNG compression method 0 is defined by this International Standard. Other values of compression method are reserved
      for future standardization. PNG compression method 0 is [deflate](#dfn-deflate) compression with a sliding window (which is an upper
      bound on the distances appearing in the [deflate](#dfn-deflate) stream) of at most 32768 bytes. [Deflate](#dfn-deflate) compression is derived
      from [LZ77](#3LZ77).
...
For PNG compression method 0, the [zlib](#dfn-zlib) compression method/flags code shall specify method code 8 ([deflate](#dfn-deflate)
      compression) and an [LZ77](#3LZ77) window size of not more than 32768 bytes. The [zlib](#dfn-zlib) compression method number is not
      the same as the PNG compression method number in the [IHDR](#11IHDR) chunk. The additional flags
...
The compressed data within the [zlib](#dfn-zlib) datastream are stored as a series of blocks, each of which can represent raw
      (uncompressed) data, [LZ77](#3LZ77)-compressed data encoded with fixed Huffman codes, or [LZ77](#3LZ77)-compressed data encoded
      ...
      [deflate](#dfn-deflate) specification [[rfc1951](#bib-rfc1951)].

The check value stored at the end of the [zlib](#dfn-zlib) datastream is calculated on the uncompressed data represented by the
      datastream. The algorithm used to calculate this is not the same as the CRC calculation used for PNG chunk CRC
      field values. The [zlib](#dfn-zlib) check value is useful mainly as a cross-check that the [deflate](#dfn-deflate) algorithms are
...
The sequence of filtered scanlines is compressed and the resulting data stream is split into [IDAT](#11IDAT) chunks. The concatenation of the contents of all the [IDAT](#11IDAT) chunks makes
      up a [zlib](#dfn-zlib) datastream. This datastream decompresses to filtered [image data](#dfn-image-data).

It is important to emphasize that the boundaries between [IDAT](#11IDAT) chunks are arbitrary and
      can fall anywhere in the [zlib](#dfn-zlib) datastream. There is not necessarily any correlation between [IDAT](#11IDAT) chunk boundaries and [deflate](#dfn-deflate) block boundaries or any other feature of the [zlib](#dfn-zlib) data. For
      example, it is entirely possible for the terminating [zlib](#dfn-zlib) check value to be split across [IDAT](#11IDAT) chunks.

Similarly, there is no required correlation between the structure of the [image data](#dfn-image-data) (i.e., scanline boundaries) and
      [deflate](#dfn-deflate) block boundaries or [IDAT](#11IDAT) chunk boundaries. The complete filtered PNG image
      is represented by a single [zlib](#dfn-zlib) datastream that is stored in a number of [IDAT](#11IDAT)
      chunks.

PNG also uses compression method 0 in [iTXt](#11iTXt), [iCCP](#11iCCP),
      and [zTXt](#11zTXt) chunks. Unlike the [image data](#dfn-image-data), such datastreams are not split across
      chunks; each such chunk contains an independent [zlib](#dfn-zlib) datastream (see [10.1 Compression method 0](#10CompressionCM0)).

This clause defines chunk used in this specification.

A critical chunk is a chunk that is absolutely required in order to successfully decode a PNG image from a PNG
        datastream. Extension chunks may be defined as critical chunks (see [14. Editors](#14EditorsExt)), though this practice is
...
`49 48 44 52`The [IHDR](#11IHDR) chunk shall be the first chunk in the PNG datastream. It contains:
...
Width and height give the image dimensions in pixels. They are [PNG four-byte unsigned integers](#dfn-png-four-byte-unsigned-integer). Zero is an
        invalid value.

Bit depth is a single-byte integer giving the number of bits per sample or per palette index (not per pixel). Valid
        values are 1, 2, 4, 8, and 16, although not all values are allowed for all [color types](#3colourType). See [6.1 Color types and values](#6Colour-values).

[Color type](#3colourType) is a single-byte integer.

Bit depth restrictions for each [color type](#3colourType) are imposed to simplify implementations and to prohibit combinations
        that do not compress well. The allowed combinations are defined in [Table 12](#table111).
...
The sample depth is the same as the bit depth except in the case of [indexed-color](#3indexedColour) PNG images ([color type](#3colourType) 3), in
        which the sample depth is always 8 bits (see [4.5 PNG image](#4Concepts.PNGImage)).

Compression method is a single-byte integer that indicates the method used to compress the [image data](#dfn-image-data). Only
        compression method 0 ([deflate](#dfn-deflate) compression with a sliding window of at most 32768 bytes) is defined in this
        specification. All conforming PNG images shall be compressed with this scheme.

Filter method is a single-byte integer that indicates the preprocessing method applied to the [image data](#dfn-image-data) before
        compression. Only [filter method](#3filter) 0 (adaptive filtering with five basic filter types) is defined in this
        specification. See [9. Filtering](#9Filters) for details.

Interlace method is a single-byte integer that indicates the transmission order of the [image data](#dfn-image-data). Two values are
        defined in this specification: 0 (no interlace) or 1 (Adam7 interlace). See [8. Interlacing and pass extraction](#8Interlace) for details.

The four-byte chunk type field contains the hexadecimal values

`50 4C 54 45`The PLTE chunk contains from 1 to 256 palette entries, each a three-byte series of the form:
...
This chunk shall appear for [color type](#3colourType) 3, and may appear for [color types](#3colourType) 2 and 6; it shall not appear
        for [color types](#3colourType) 0 and 4. There shall not be more than one PLTE chunk.

For [color type](#3colourType) 3 (indexed-color), the PLTE chunk is required. The first entry in
        PLTE is referenced by pixel value 0, the second by pixel value 1, etc. The number of palette
        entries shall not exceed the range that can be represented in the image bit depth (for example, 24 = 16 for a
        bit depth of 4). It is permissible to have fewer entries than the bit depth would allow. In that case, any out-of-range
        pixel value found in the [image data](#dfn-image-data) is an error.

For [color types](#3colourType) 2 and 6 ([truecolor](#3truecolour) and [truecolor with alpha](#dfn-truecolor-with-alpha)), the PLTE chunk
        is optional. If present, it provides a suggested set of colors (from 1 to 256) to which the [truecolor](#3truecolour) image can be
        quantized if it cannot be displayed directly. It is, however, recommended that the [sPLT](#11sPLT)
        chunk be used for this purpose, rather than the PLTE chunk. If neither PLTE nor [sPLT](#11sPLT) chunks are present and the image cannot be displayed
        directly, quantization has to be done by the viewing system. However, it is often preferable for the selection of colors
        to be done once by the PNG encoder. (See [12.5 Suggested palettes](#12Suggested-palettes).)

Note that the palette uses 8 bits (1 byte) per sample regardless of the image bit depth. In particular, the palette is 8
        bits deep even when it is a suggested quantization of a 16-bit [truecolor](#3truecolour) image.

There is no requirement that the palette entries all be used by the image, nor that they all be different.

The four-byte chunk type field contains the hexadecimal values

`49 44 41 54`The IDAT chunk contains the actual [image data](#dfn-image-data) which is the output stream of the
        compression algorithm. See [9. Filtering](#9Filters) and [10. Compression](#10Compression) for details.

There may be multiple IDAT chunks; if so, they shall appear consecutively with no other
        intervening chunks. The compressed datastream is then the concatenation of the contents of the data fields of all the
        IDAT chunks
        (noting that data fields [may be of zero length](#zero-length-data)).
...
`49 45 4E 44`The IEND chunk marks the end of the PNG datastream. The chunk's data field is empty.

The ancillary chunks defined in this specification are listed in the order in [4.8.2 Chunk types](#4Concepts.FormatTypes).
...
`74 52 4E 53`The tRNS chunk specifies either alpha values that are associated with palette entries (for
          [indexed-color](#3indexedColour) images) or a single transparent color (for [greyscale](#dfn-greyscale) and [truecolor](#3truecolour) images). The tRNS chunk contains:
...
For [color type](#3colourType) 3 (indexed-color), the tRNS chunk contains a series of one-byte
          alpha values, corresponding to entries in the [PLTE](#11PLTE) chunk. Each entry indicates that
          ...
          bit depth. The tRNS chunk shall not contain more alpha values than there are palette entries,
          but a tRNS chunk may contain fewer values than there are palette entries. In this case, the
          alpha value for all remaining palette entries is assumed to be 255. In the common case in which only palette index 0 need
          be made transparent, only a one-byte tRNS chunk is needed, and when all palette indices are
          opaque, the tRNS chunk may be omitted.

For [color types](#3colourType) 0 or 2, two bytes per sample are used regardless of the image bit depth (see [7.1 Integers and byte order](#7Integers-and-byte-order)). Pixels of the specified grey sample value or RGB sample values are treated as
...
A tRNS chunk shall not appear for [color types](#3colourType) 4 and 6, since a full alpha channel
          is already present in those cases.

Note

NOTE For 16-bit [greyscale](#dfn-greyscale) or [truecolor](#3truecolour) data,
          as described in [13.12 Sample depth rescaling](#13Sample-depth-rescaling),
          only [pixels matching the entire 16-bit values](#tRNS-compare-exactly) in
          tRNS chunks are transparent. Decoders have to postpone any sample depth rescaling until after
          the pixels have been tested for transparency.

The four-byte chunk type field contains the hexadecimal values

`63 48 52 4D`The cHRM chunk may be used to specify the 1931 CIE *x,y* chromaticities of the red,
          green, and blue display primaries used in the [PNG image](#3PNGimage), and the referenced [white point](#dfn-white-point). See [C. Gamma and chromaticity](#C-GammaAppendix) for more information. The [iCCP](#11iCCP), and [sRGB](#11sRGB) chunks provide more sophisticated support for color management and control.
...
Each value is encoded as a [PNG four-byte unsigned integer](#dfn-png-four-byte-unsigned-integer), representing the *x* or *y* value times
          100000.

A value of 0.3127 would be stored as the integer 31270.

The cHRM chunk is allowed in all PNG datastreams, although it is of little value for
          [greyscale](#dfn-greyscale) images.

This chunk is ignored
            unless it is the [highest-precedence color chunk](#color-chunk-precendence)
            understood by the decoder.

The four-byte chunk type field contains the hexadecimal values

`67 41 4D 41`The [gAMA](#11gAMA) chunk specifies a [gamma value](#dfn-gamma-value).

In fact specifying the desired display output intensity is insufficient. It is also necessary to specify the viewing
          conditions under which the output is desired. For gAMA these are the reference viewing
          conditions of the sRGB specification [[SRGB](#bib-srgb)]. Adjustment for different viewing conditions is normally handled by a
          Color Management System. If the adjustment is not performed, the error is usually small. Applications desiring high
          color fidelity may wish to use an [sRGB](#11sRGB), [iCCP](#11iCCP)
          chunk.
...
`69 43 43 50`The iCCP chunk contains:
...
The profile name may be any convenient name for referring to the profile. It is case-sensitive. Profile names shall
          ...
          ([zlib](#dfn-zlib) datastream with [deflate](#dfn-deflate) compression, see [10.3 Other uses of compression](#10CompressionOtherUses)). The compression
...
If the iCCP chunk is present, the image samples conform to the color space represented by
          the embedded ICC profile as defined by the International Color Consortium [[ICC](#bib-icc)][[ISO_15076-1](#bib-iso_15076-1)]. The color space of the
          ICC profile shall be an RGB color space for color images ([color types](#3colourType) 2, 3, and 6), or a greyscale color space
          for [greyscale](#dfn-greyscale) images ([color types](#3colourType) 0 and 4). A PNG encoder that writes the iCCP chunk
          is encouraged to also write [gAMA](#11gAMA) and [cHRM](#11cHRM) chunks
          that approximate the ICC profile, to provide compatibility with applications that do not use the iCCP chunk.
...
Each depth specified in sBIT shall be greater than zero and less than or equal to the
          sample depth (which is 8 for [indexed-color](#3indexedColour) images, and the bit depth given in [IHDR](#11IHDR)
          for other [color types](#3colourType)). Note that sBIT does not provide a sample depth for the alpha
          channel that is implied by a [tRNS](#11tRNS) chunk; in that case, all of the sample bits of the
          alpha channel are to be treated as significant. If the sBIT chunk is not present, then all of
          the sample bits of all channels are to be treated as significant.

The four-byte chunk type field contains the hexadecimal values

`73 52 47 42`If the sRGB chunk is present, the image samples conform to the sRGB color space [[SRGB](#bib-srgb)]
          and should be displayed using the specified rendering intent defined by the International Color Consortium [[ICC](#bib-icc)] or
          [[ICC-2](#bib-icc-2)].
...
It is recommended that a PNG encoder that writes the sRGB chunk also write a [gAMA](#11gAMA) chunk (and optionally a [cHRM](#11cHRM) chunk) for compatibility
          with decoders that do not use the sRGB chunk. Only the following values shall be used.
...
It is recommended that the [sRGB](#11sRGB) and [iCCP](#11iCCP)
          chunks do not appear simultaneously in a PNG datastream.

The four-byte chunk type field contains the hexadecimal values

`63 49 43 50`If present, the cICP chunk specifies the color space (primaries), transfer function, matrix
          coefficients and scaling factor of the image using the code points specified in [[ITU-T-H.273](#bib-itu-t-h.273)]. The video format signaling *SHOULD* be used
...
Each of the fields of the cICP chunk corresponds to the parameter of the same name in
          [[ITU-T-H.273](#bib-itu-t-h.273)].

RGB is currently the only supported color model in PNG, and as such `Matrix Coefficients` shall be set to `0`.

Note

If `Video Full Range Flag` is `0`
            (a [narrow-range image](#dfn-narrow-range-image)), recommended practice
            is to define transfer functions
            such as [EOTF](#dfn-eotf) or inverse [OETF](#dfn-oetf)
            over the extended range,
            so as to include negative values.
            This is done as follows:


`out = sign(in) * TransferFunction(abs(in))`The cICP chunk *MUST* come before the [PLTE](#11PLTE) and [IDAT](#11IDAT) chunks.

            This chunk, if understood by the decoder, is the
            [highest-precedence color chunk](#color-chunk-precendence).


In a similar way to the use of the
          [sRGB](#srgb-standard-colour-space) chunk
          to compactly signal an sRGB image, cICP can be used
          to compactly signal a Display P3 image [[Display-P3](#bib-display-p3)].


The four-byte chunk type field contains the hexadecimal values

`6D 44 43 56`If present, the mDCV chunk characterizes
          the Mastering Display Color Volume (mDCV) used at the point of content creation,
          as specified in [[SMPTE-ST-2086](#bib-smpte-st-2086)]. The mDCV chunk provides informative static metadata which
...
mDCV is typically used with the [PQ](#dfn-pq) [[ITU-R-BT.2100](#bib-itu-r-bt.2100)] transfer function
          and additional [cLLI](#cLLI-chunk) metadata and is commonly then called
          [[HDR10](#bib-hdr10)] (PQ with ST 2086 static metadata, MaxFALL and MaxCLL).
          The mDCV chunk may also be included with [HLG](#dfn-hlg) [[ITU-R-BT.2100](#bib-itu-r-bt.2100)] and
          [SDR](#dfn-sdr) image formats (for example [[ITU-R-BT.709](#bib-itu-r-bt.709)]).
...
Since mDCV was originally created as supplemental static metadata meant to
          ...
          both HDR [[ITU-R-BT.2100](#bib-itu-r-bt.2100)] and SDR [[ITU-R-BT.709](#bib-itu-r-bt.709)] are available in
          [[ITU-T-Series-H-Supplement-19](#bib-itu-t-series-h-supplement-19)]. The basic (cICP) characteristics plus the supplemental
          (mDCV) static metadata may provide valuable information to optimize
          tone-mapping decisions.

Note

[Issue #319](https://github.com/w3c/png/issues/319) discusses tone-mapping behavior when
          the mDCV chunk is present.

For [SDR](#dfn-sdr) images, if mDCV display min/max luminance are unknown, the default
          characteristics can be derived from the values in [[ITU-T-Series-H-Supplement-19](#bib-itu-t-series-h-supplement-19)] Table 11 or from the relevant [SDR](#dfn-sdr) specification.
...
|---|---|---|
| Mastering display color primary chromaticities (CIE 1931 x,yof R,G,B ) | 12 bytes | 0.00002 |
| Mastering display white point chromaticity (CIE 1931 x,y) | 4 bytes | 0.00002 |
| Mastering display maximum luminance (measured in cd/m 2) | 4 bytes | 0.0001 |
| Mastering display minimum luminance (measured in cd/m 2) | 4 bytes | 0.0001 |

The color primaries are encoded as
            three pairs of [PNG two-byte unsigned integer](#dfn-png-two-byte-unsigned-integer)s,
            in the order *x* and then *y*,
            ...
            a pair of [PNG two-byte unsigned integer](#dfn-png-two-byte-unsigned-integer)s,
            in the order *x* and then *y*,
            ...
            [PNG four-byte unsigned integer](#dfn-png-four-byte-unsigned-integer)s,
            representing the absolute luminance value in cd/m2
...
`63 4C 4C 49`If present, the cLLI chunk identifies two characteristics of [HDR](#dfn-hdr) content:
...
MaxCLL (Maximum Content Light Level) uses a static metadata value to indicate the maximum light
            level of any single pixel (in cd/m2, also known as nits) of the entire playback sequence.
...
MaxFALL (Maximum Frame Average Light Level) uses a static metadata value to indicate the
            maximum value of the [frame](#dfn-frame) average light level (in cd/m2, also known as nits)
...
[[CTA-861.3-A](#bib-cta-861.3-a)] describes the method of calculation for generating the
            cLLI values,
            but does not specify any filtering.
            [[HDR-Static-Meta](#bib-hdr-static-meta)] describes an improved method which rejects extreme values from
...
[[SMPTE-ST-2067-21](#bib-smpte-st-2067-21)] Section 7.5 adds additional
            information in Section 7.5 in the case where the cLLI values are unknown and have not been calculated.

Note

[Issue #319](https://github.com/w3c/png/issues/319) discusses tone-mapping behavior when
          the cLLI chunk is present.
...
PNG provides the [tEXt](#11tEXt), [iTXt](#11iTXt), and [zTXt](#11zTXt) chunks for storing text strings associated with the image, such as an image description
...
| XML:com.adobe.xmp | Extensible Metadata Platform (XMP) information, formatted as required by the XMP specification [ [XMP](#bib-xmp)]. The use of[iTXt](#11iTXt), with Compression Flag set to 0, and both Language Tag and Translated
...
Keywords shall contain only printable Latin-1 [[ISO_8859-1](#bib-iso_8859-1)] characters and spaces; that is, only code points 0x20-7E
...
The [iTXt](#11iTXt) chunk uses the UTF-8 encoding [[rfc3629](#bib-rfc3629)] and can be used to convey
          characters in any language. There is an option to compress text strings in the [iTXt](#11iTXt)
          chunk. [iTXt](#11iTXt) is recommended for all text strings, as it supports Unicode. There are
          also [tEXt](#11tEXt) and [zTXt](#11zTXt) chunks, whose content is
          restricted to the printable Latin-1 character set plus U+000A LINE FEED (LF). Text strings in [zTXt](#11zTXt) are compressed into [zlib](#dfn-zlib) datastreams using [deflate](#dfn-deflate) compression (see [10.3 Other uses of compression](#10CompressionOtherUses)).

The four-byte chunk type field contains the hexadecimal values

`74 45 58 74`Each tEXt chunk contains a keyword and a text string, in the format:
...
The keyword and text string are separated by a zero byte (null character). Neither the keyword nor the text string may
          contain a null character. The text string is **not** null-terminated (the length of the chunk defines the
...
Text is interpreted according to the Latin-1 character set [[ISO_8859-1](#bib-iso_8859-1)]. The text string may contain any Latin-1
          character. Newlines in the text string should be represented by a single linefeed character (decimal 10). Characters
          other than those defined in Latin-1 plus the linefeed character have no defined meaning in tEXt chunks. Text containing characters outside the repertoire of ISO/IEC 8859-1 should be encoded using
          the [iTXt](#11iTXt) chunk.

The four-byte chunk type field contains the hexadecimal values

`7A 54 58 74`The zTXt and [tEXt](#11tEXt) chunks are semantically equivalent,
          but the zTXt chunk is recommended for storing large blocks of text.
...
The keyword and null character are the same as in the [tEXt](#11tEXt) chunk. The keyword is not compressed. The compression method entry defines the compression method used. The
          only value defined in this International Standard is 0 ([deflate](#dfn-deflate) compression). Other values are reserved for future
          standardization. The compression method entry is followed by the compressed text datastream that makes up the remainder
          of the chunk. For compression method 0, this datastream is a [zlib](#dfn-zlib) datastream with deflate compression (see
          [10.3 Other uses of compression](#10CompressionOtherUses)). Decompression of this datastream yields Latin-1 text that is identical to the
          text that would be stored in an equivalent [tEXt](#11tEXt) chunk.

The four-byte chunk type field contains the hexadecimal values

`69 54 58 74`An iTXt chunk contains:
...
The compression flag is 0 for uncompressed text, 1 for compressed text. Only the text field may be compressed. The
          ...
          is 0 ([zlib](#dfn-zlib) datastream with [deflate](#dfn-deflate) compression, see [10.3 Other uses of compression](#10CompressionOtherUses)). For
          uncompressed text, encoders shall set the compression method to 0, and decoders shall ignore it.

The language tag is a well-formed language tag defined by [[BCP47](#bib-bcp47)]. Unlike the keyword, the language tag is
          case-insensitive. Subtags must appear in the
          [IANA language subtag registry](https://www.iana.org/assignments/language-subtag-registry/language-subtag-registry).
          If the language tag is empty, the language is
          unspecified. Examples of language tags include: `en`, `en-GB`, `es-419`,
          `zh-Hans`, `zh-Hans-CN`, `tlh-Cyrl-AQ`, `ar-AE-u-nu-latn`, and
          `x-private`.

The translated keyword and text both use the UTF-8 encoding [[rfc3629](#bib-rfc3629)], and neither shall contain a zero byte (null
...
`62 4B 47 44`The bKGD chunk specifies a default background color to present the image against. If there is any other preferred background, either user-specified or part of a larger page (as in a browser), the bKGD chunk should be ignored. The bKGD chunk contains:
...
For [color types](#3colourType) 0 and 4 ([greyscale](#dfn-greyscale), [greyscale with alpha](#dfn-greyscale-with-alpha)), the value is the grey level to be used as
          background in the range 0 to (2bitdepth)-1. For [color types](#3colourType) 2 and 6 ([truecolor](#3truecolour), [truecolor
          with alpha](#dfn-truecolor-with-alpha)), the values are the color to be used as background, given as RGB samples in the range 0 to
          (2bitdepth)-1. In each case, for consistency, two bytes per sample are used regardless of the image bit depth.
          If the image bit depth is less than 16, the least significant bits are used.
          Encoders should set the other bits to 0, and decoders must mask the other bits to 0 before the value is used.

The four-byte chunk type field contains the hexadecimal values

`68 49 53 54`The hIST chunk contains a series of two-byte unsigned integers:

| Frequency | 2 bytes (unsigned integer) |
| ...etc... |

The hIST chunk gives the approximate usage frequency of each color in the palette. A
          histogram chunk can appear only when a [PLTE](#11PLTE) chunk appears. If a viewer is unable to
          provide all the colors listed in the palette, the histogram may help it decide how to choose a subset of the colors for
          display.

There shall be exactly one entry for each entry in the [PLTE](#11PLTE) chunk. Each entry is
...
NOTE When the palette is a suggested quantization of a [truecolor](#3truecolour) image, the histogram is
...
`70 48 59 73`The pHYs chunk specifies the intended pixel size or aspect ratio for display of the image. It contains:
...
`73 50 4C 54`The sPLT chunk contains:
...
The palette name is case-sensitive, and subject to the same restrictions as the keyword parameter for the [tEXt](#11tEXt) chunk. Palette names shall contain only printable Latin-1 characters and spaces (only
...
The red, green, blue, and alpha samples are either one or two bytes each, depending on the sPLT sample depth, regardless of the image bit depth. The color samples are not premultiplied by alpha,
          nor are they precomposited against any background. An alpha value of 0 means fully transparent. An alpha value of 255
          (when the sPLT sample depth is 8) or 65535 (when the sPLT sample
          depth is 16) means fully opaque. The sPLT chunk may appear for any [color type](#3colourType). Entries
          in sPLT use the same [gamma value](#dfn-gamma-value) and [chromaticity](#dfn-chromaticity) values as the PNG image, but
          may fall outside the range of values used in the color space of the PNG image; for example, in a [greyscale](#dfn-greyscale) PNG image,
          each sPLT entry would typically have equal red, green, and blue values, but this is not
          required. Similarly, sPLT entries can have non-opaque alpha values even when the PNG image
...
Each frequency value is proportional to the fraction of the pixels in the image for which that palette entry is the
          closest match in RGBA space, before the image has been [composited](#dfn-composited) against any background. The exact scale factor
...
`65 58 49 66`The data segment of the eXIf chunk contains an Exif profile in the format specified in
          "4.7.2 Interoperability Structure of APP1 in Compressed Data" of [[CIPA-DC-008](#bib-cipa-dc-008)] except that the JPEG APP1 marker,
          length, and the "Exif ID code" described in 4.7.2(C), i.e., "Exif", NULL, and padding byte, are not included.

The
          eXIf chunk size is constrained only by the maximum of 231-1 bytes imposed by the
          PNG specification. Only one eXIf chunk is allowed in a PNG datastream.

The eXIf chunk contains metadata concerning the original [image data](#dfn-image-data). If the image
          has been edited subsequent to creation of the Exif profile, this data might no longer apply to the PNG [image data](#dfn-image-data).
...
While the PNG specification allows the chunk size to be as large as 231-1 bytes, application authors
            should be aware that, if the Exif profile is going to be written to a JPEG [[JPEG](#bib-jpeg)] datastream, the total length of the
            eXIf chunk data may need to be adjusted to not exceed 216-9 bytes, so it can fit
...
`49 49 2A 00 (ASCII "II", 16-bit little-endian integer 42)`or

`4D 4D 00 2A (ASCII "MM", 16-bit big-endian integer 42)`All other values are reserved for possible future definition.

Image editing applications should consider Paragraph E.3 of the Exif Specification [[CIPA-DC-008](#bib-cipa-dc-008)], which discusses
...
`74 49 4D 45`The tIME chunk gives the time of the last image modification (**not** the time
          of initial image creation). It contains:

| Name | Size |
|---|---|
| Year | 2 bytes (complete; for example, 1995, not95) |
| Month | 1 byte (1-12) |
| Day | 1 byte (1-31) |
| Hour | 1 byte (0-23) |
| Minute | 1 byte (0-59) |
...
The tIME chunk is intended for use as an automatically-applied time stamp that is updated
          whenever the [image data](#dfn-image-data) are changed.

The four-byte chunk type field contains the hexadecimal values

`61 63 54 4C`The acTL chunk declares that this is an animated PNG image, gives the number of frames, and the number of times to loop. It contains:
...
`num_frames` indicates the total number of frames in the animation. This must equal the number of [fcTL](#fcTL-chunk) chunks. 0 is not a valid value. 1 is a valid value, for a single-frame PNG. If this value
          does not equal the actual number of frames it should be treated as an error.

`num_plays` indicates the number of times that this animation should play; if it is 0, the animation should play
          indefinitely. If nonzero, the animation should come to rest on the final frame at the end of the last play.

The acTL chunk must appear before the first [IDAT](#11IDAT) chunk
...
`66 63 54 4C`The fcTL chunk defines the dimensions, position, delay and disposal of an individual frame. Exactly one fcTL chunk chunk is required for each frame. It contains:
...
The frame must be rendered within the region defined by `x_offset`, `y_offset`, `width`, and `height`. This region may
          not fall outside of the default image; thus `x_offset` plus `width` must not be greater than the [IHDR](#11IHDR) width; similarly `y_offset` plus `height` must not be greater than the [IHDR](#11IHDR) height.

`delay_num` and `delay_den` define the numerator and denominator of the delay fraction; indicating the time to display
...
`dispose_op` defines the type of frame area disposal to be done after rendering this frame; in other words, it
...
If the first fcTL chunk uses a `dispose_op` of `APNG_DISPOSE_OP_PREVIOUS` it should be
          treated as `APNG_DISPOSE_OP_BACKGROUND`.

`blend_op` specifies whether the frame is to be alpha blended into the current output buffer content, or whether it
...
If `blend_op` is `APNG_BLEND_OP_SOURCE` all color components of the frame, including alpha, overwrite the current
          contents of the frame's output buffer region. If `blend_op` is `APNG_BLEND_OP_OVER` the frame should be [composited](#dfn-composited)
          onto the output buffer based on its alpha, using a simple OVER operation as described in [Alpha Channel Processing](#13Alpha-channel-processing). Note that the second variation of the sample code is
          applicable.
...
`66 64 41 54`The fdAT chunk serves the same purpose for animations as the [IDAT](#11IDAT) chunks do for static images;
          the set of fdAT chunks
          contains the [image data](#dfn-image-data) for all frames (or, for animations
          which include the [static image](#dfn-static-image) as first frame, for all frames after the first one). It contains:
...
At least one fdAT chunk is required for each frame, except for the first frame, if that
          frame is represented by an [IDAT](#11IDAT) chunk.

The compressed datastream for each frame is then the concatenation,
          in ascending [sequence number](#4Concepts.APNGSequence) order,
          of the contents of the `frame_data` fields of all the fdAT chunks within a frame.

Because of the sequence number, fdAT chunks
          [may not be of zero length](#zero-length-data));
          ...
          including the filter byte at the beginning of each scanline, similar to the uncompressed data of all the [IDAT](#11IDAT) chunks. It utilizes the same bit depth, [color type](#3colourType), compression method, [filter
...
If the PNG [pHYs](#11pHYs) chunk is present, the APNG images and their `x_offset` and
          `y_offset` values must be scaled in the same way as the main image. Conceptually, such scaling occurs while mapping the
          output buffer onto the [canvas](#dfn-canvas).
...
PNG encoders capable of full color management will perform more sophisticated calculations than those described here and
      may choose to use the [iCCP](#11iCCP) chunk. If it is known that the image samples conform to the
      sRGB specification [[SRGB](#bib-srgb)], encoders are strongly encouraged to write the [sRGB](#srgb-standard-colour-space) chunk without performing additional [gamma](#dfn-gamma) handling. In both cases it is
      recommended that an appropriate [gAMA](#11gAMA) chunk be generated for use by PNG decoders that do
      not recognize the [iCCP](#11iCCP) or [sRGB](#srgb-standard-colour-space)
      chunks.

A PNG encoder has to determine:

- what value to write in the [gAMA](#11gAMA)chunk;
- how to transform the provided image samples into the values to be written in the PNG datastream.

The value to write in the [gAMA](#11gAMA) chunk is that value which causes a PNG decoder to
      behave in the desired way. See [13.13 Decoder gamma handling](#13Decoder-gamma-handling).
...
`integer_sample = floor((2`
      sampledepth-1) * intensityencoding_exponent + 0.5)

If the intensity in the equation is the desired output intensity, the encoding exponent is the [gamma value](#dfn-gamma-value) to be
      used in the [gAMA](#11gAMA) chunk.
...
If the intensity available to the PNG encoder is the original scene intensity, another transformation may be needed. There
      ...
      to an end-to-end [transfer function](#dfn-transfer-function) from original scene to display output with an exponent greater than 1. In this
      case:

`gamma = encoding_exponent/end_to_end_exponent`If it is not known whether the conditions under which the original image was captured or calculated warrant such a contrast change, it may be assumed that the display intensities are proportional to original scene intensities, i.e. the end-to-end exponent is 1 and hence:

`gamma = encoding_exponent`If the image is being written to a datastream only, the encoder is free to choose the encoding exponent. Choosing a value
      that causes the [gamma value](#dfn-gamma-value) in the [gAMA](#11gAMA) chunk to be 1/2.2 is often a reasonable
...
Some image renderers may simultaneously write the image to a PNG datastream and display it on-screen. The displayed pixels
      should be [gamma](#dfn-gamma) corrected for the display system and viewing conditions in use, so that the user sees a proper
...
If the renderer wants to write the displayed sample values to the PNG datastream, avoiding a separate [gamma
      encoding](#dfn-gamma-encoding) step for the datastream, the renderer should approximate the [transfer function](#dfn-transfer-function) of the display system by a
      power function, and write the reciprocal of the exponent into the [gAMA](#11gAMA) chunk. This will
...
However, it is equally reasonable for a renderer to compute displayed pixels appropriate for the display device, and to
      perform separate [gamma encoding](#dfn-gamma-encoding) for data storage and transmission, arranging to have a value in the [gAMA](#11gAMA) chunk more appropriate to the future use of the image.

Computer graphics renderers often do not perform [gamma encoding](#dfn-gamma-encoding), instead making sample values directly proportional
      to scene light intensity. If the PNG encoder receives sample values that have already been quantized into integer values,
      there is no point in doing [gamma encoding](#dfn-gamma-encoding) on them; that would just result in further loss of information. The encoder
      should just write the sample values to the PNG datastream. This does not imply that the [gAMA](#11gAMA) chunk should contain a [gamma value](#dfn-gamma-value) of 1.0 because the desired end-to-end [transfer function](#dfn-transfer-function)
      from scene intensity to display output intensity is not necessarily linear. However, the desired [gamma value](#dfn-gamma-value) is
      probably not far from 1.0. It may depend on whether the scene being rendered is a daylight scene or an indoor scene, etc.

When the sample values come directly from a piece of hardware, the correct [gAMA](#11gAMA) value
      can, in principle, be inferred from the [transfer function](#dfn-transfer-function) of the hardware and lighting conditions of the scene. In the
      ...
      that [gamma](#dfn-gamma) relates samples to desired display output, not to scanner input.
...
Datastream format converters generally should not attempt to convert supplied images to a different [gamma](#dfn-gamma). The data
      should be stored in the PNG datastream without conversion, and the [gamma value](#dfn-gamma-value) should be deduced from information in
      the source datastream if possible. [Gamma](#dfn-gamma) alteration at datastream conversion time causes re-quantization of the set of
...
If the source datastream describes the [gamma](#dfn-gamma) characteristics of the image, a datastream converter is strongly
      encouraged to write a [gAMA](#11gAMA) chunk. Some datastream formats specify the display exponent
      (the exponent of the function which maps image samples to display output rather than the other direction). If the source
      file's [gamma value](#dfn-gamma-value) is greater than 1.0, it is probably a display exponent, and the reciprocal of this value should be
      used for the PNG [gamma value](#dfn-gamma-value). If the source file format records the relationship between image samples and a quantity
      other than display output, it will be more complex than this to deduce the PNG [gamma value](#dfn-gamma-value).

If a PNG encoder or datastream converter knows that the image has been displayed satisfactorily using a display system
      whose [transfer function](#dfn-transfer-function) can be approximated by a power function with exponent `display_exponent`, the image
      can be marked as having the [gamma value](#dfn-gamma-value):

`gamma = 1/display_exponent`It is better to write a [gAMA](#11gAMA) chunk with a value that is approximately correct than to
      omit the chunk and force PNG decoders to guess an approximate [gamma value](#dfn-gamma-value). If a PNG encoder is unable to infer the
      [gamma value](#dfn-gamma-value), it is preferable to omit the [gAMA](#11gAMA) chunk. If a guess has to be made
...
PNG encoders capable of full color management will perform more sophisticated calculations than those described here and
      may choose to use the [iCCP](#11iCCP) chunk. If it is known that the image samples conform to the
      sRGB specification [[SRGB](#bib-srgb)], PNG encoders are strongly encouraged to use the [sRGB](#srgb-standard-colour-space) chunk.
...
If it is possible for the encoder to determine the chromaticities of the source display primaries, or to make a strong
      ...
      [cHRM](#11cHRM) chunk. If this is done, the [gAMA](#11gAMA) chunk should
      also be written; decoders can do little with a [cHRM](#11cHRM) chunk if the [gAMA](#11gAMA) chunk is missing.

There are a number of recommendations and standards for primaries and [white points](#dfn-white-point), some of which are linked to
      particular technologies, for example the CCIR 709 standard [[ITU-R-BT.709](#bib-itu-r-bt.709)] and the SMPTE-C standard [[SMPTE-170M](#bib-smpte-170m)].
...
In the case of hand-drawn or digitally edited images, it is necessary to determine what monitor they were viewed on when
      ...
      are working in some device-independent space internally. Such programs have enough information to write valid [cHRM](#11cHRM) and [gAMA](#11gAMA) chunks, and are strongly encouraged to do so
      automatically.
...
If the encoder is compiled as a portion of a computer image renderer that performs full-spectral rendering, the monitor
      ...
      [cHRM](#11cHRM) chunk. Any colors that are outside the gamut of the chosen RGB device should be
      mapped to be within the gamut; PNG does not store out-of-gamut colors.

If the computer image renderer performs calculations directly in device-dependent RGB space, a [cHRM](#11cHRM) chunk should not be written unless the scene description and rendering parameters have been adjusted for a
      particular monitor. In that case, the data for that monitor should be used to construct a [cHRM](#11cHRM) chunk.

A few image formats store calibration information, which can be used to fill in the [cHRM](#11cHRM) chunk. For example, TIFF 6.0 files [[TIFF-6.0](#bib-tiff-6.0)] can optionally store calibration information, which if
      present should be used to construct the [cHRM](#11cHRM) chunk.
...
It is **not** recommended that datastream format converters attempt to convert supplied images to a different
      ...
      gamut mismatches and rounding errors. As with [gamma](#dfn-gamma) conversions, it is better to store the data losslessly and incur
...
Image authors should keep in mind the possibility that a decoder will not support transparency control in full (see
      [13.16 Alpha channel processing](#13Alpha-channel-processing)). Hence, the colors assigned to transparent pixels should be reasonable
...
For applications that do not require a full alpha channel, or cannot afford the price in compression efficiency, the
      [tRNS](#11tRNS) transparency chunk is also available.

If the image has a known background color, this color should be written in the [bKGD](#11bKGD)
      chunk. Even decoders that ignore transparency may use the [bKGD](#11bKGD) color to fill unused
      screen area.
...
`output = floor((input * MAXOUTSAMPLE / MAXINSAMPLE) + 0.5)`where the input samples range from 0 to `MAXINSAMPLE` and the outputs range from 0 to `MAXOUTSAMPLE`
...
A distinctly less accurate approximation is obtained by simply left-shifting the input value and filling the low order
      ...
      zero-fill scaling is small at high sample depths, some encoders may choose to use it. Zero-fill shall **not** be
...
When the encoder writes an [sBIT](#11sBIT) chunk, it is required to do the scaling in such a way
      that the high-order bits of the stored samples match the original data. That is, if the [sBIT](#11sBIT) chunk specifies a sample depth of S, the high-order S bits of the stored data shall agree with the
...
When scaling up source [image data](#dfn-image-data), it is recommended that the low-order bits be filled consistently for all
...
In some applications the original source data may have a range that is not a power of 2. The linear scaling equation still
      works for this case, although the shifting methods do not. It is recommended that an [sBIT](#11sBIT)
      chunk not be written for such images, since [sBIT](#11sBIT) suggests that the original data range
      was exactly 0..2S-1.

Suggested palettes may appear as [sPLT](#11sPLT) chunks in any PNG datastream, or as a [PLTE](#11PLTE) chunk in [truecolor](#3truecolour) PNG datastreams. In either case, the suggested palette is not an
      essential part of the [image data](#dfn-image-data), but it may be used to present the image on indexed-color display hardware.
      Suggested palettes are of no interest to viewers running on [truecolor](#3truecolour) hardware.

When an [sPLT](#11sPLT) chunk is used to provide a suggested palette, it is recommended that the
      ...
      be computed for the un-[composited](#dfn-composited) image.

Even for indexed-color images, [sPLT](#11sPLT) can be used to define alternative reduced
      palettes for viewers that are unable to display all the colors present in the [PLTE](#11PLTE)
      chunk. If the [PLTE](#11PLTE) chunk appears without the [bKGD](#11bKGD)
      chunk in an image of [color type](#3colourType) 6, the circumstances under which the palette was computed are unspecified.

An older method for including a suggested palette in a [truecolor](#3truecolour) PNG datastream uses the [PLTE](#11PLTE) chunk. If this method is used, the histogram (frequencies) should appear in a separate [hIST](#11hIST) chunk. The [PLTE](#11PLTE) chunk does not include transparency information.
      Hence for images of [color type](#3colourType) 6 ([truecolor with alpha](#dfn-truecolor-with-alpha)), it is recommended that a [bKGD](#11bKGD) chunk appear and that the palette and histogram be computed with reference to the image as it would appear
      ...
      present the image against the same background color. It is recommended that [PNG editors](#dfn-png-editor) delete or recompute the
      palette if they alter or remove the [bKGD](#11bKGD) chunk in an image of [color type](#3colourType) 6.

For images of [color type](#3colourType) 2 ([truecolor](#3truecolour)), it is recommended that the [PLTE](#11PLTE) and [hIST](#11hIST) chunks be computed with reference to the RGB data only,
      ignoring any transparent-color specification. If the datastream uses transparency (has a [tRNS](#11tRNS) chunk), viewers can easily adapt the resulting palette for use with their intended background color (see
      [13.17 Histogram and suggested palette usage](#13Histogram-and-suggested-palette-usage)).

For providing suggested palettes, the [sPLT](#11sPLT) chunk is more flexible than the [PLTE](#11PLTE) chunk in the following ways:

- With [sPLT](#11sPLT)multiple suggested palettes may be provided. A PNG decoder may choose an appropriate palette based on name or number of entries.
- In a PNG datastream of [color type](#3colourType)6 ([truecolor with alpha](#dfn-truecolor-with-alpha)channel), the[PLTE](#11PLTE)chunk represents a palette already[composited](#dfn-composited)against the[bKGD](#11bKGD)color, so it is useful only for display against that background color. The[sPLT](#11sPLT)chunk provides an un-[composited](#dfn-composited)palette, which is useful for display against backgrounds chosen by the PNG decoder.
- Since the [sPLT](#11sPLT)chunk is an ancillary chunk, a[PNG editor](#dfn-png-editor)may add or modify suggested palettes without being forced to discard unknown unsafe-to-copy chunks.
- Whereas the [sPLT](#11sPLT)chunk is allowed in PNG datastreams for[color types](#3colourType)0, 3, and 4 ([greyscale](#dfn-greyscale)and[indexed-color](#3indexedColour)), the[PLTE](#11PLTE)chunk cannot be used to provide reduced palettes in these cases.
- More than 256 entries may appear in the [sPLT](#11sPLT)chunk.

A PNG encoder that uses the [sPLT](#11sPLT) chunk may choose to write a suggested palette
      represented by [PLTE](#11PLTE) and [hIST](#11hIST) chunks as well, for
      compatibility with decoders that do not recognize the [sPLT](#11sPLT) chunk.

This specification defines two interlace methods, one of which is no interlacing. Interlacing provides a convenient basis
      from which decoders can progressively display an image, as described in [13.10 Interlacing and progressive display](#13Progressive-display).

For images of [color type](#3colourType) 3 (indexed-color), filter type 0 (None) is usually the most effective. Color images
      with 256 or fewer colors should almost always be stored in [indexed-color](#3indexedColour) format; [truecolor](#3truecolour) format is likely to be
      much larger.
...
For [truecolor](#3truecolour) and [greyscale](#dfn-greyscale) images, any of the five filters may prove the most effective. If an encoder uses a
      fixed filter, the Paeth filter type is most likely to be the best.

For best compression of [truecolor](#3truecolour) and [greyscale](#dfn-greyscale) images,
...
The encoder may divide the compressed datastream into [IDAT](#11IDAT) chunks however it wishes.
      (Multiple [IDAT](#11IDAT) chunks are allowed so that encoders may work in a fixed amount of memory;
      typically the chunk size will correspond to the encoder's buffer size.) A PNG datastream in which each [IDAT](#11IDAT) chunk contains only one data byte is valid, though remarkably wasteful of space. ([Zero-length](#zero-length-data) [IDAT](#11IDAT) chunks are also valid, though even more wasteful.)
...
The [iTXt](#11iTXt) chunk uses the UTF-8 encoding of Unicode and thus can store text in any
      language. The [tEXt](#11tEXt) and [zTXt](#11zTXt) chunks use the Latin-1
      (ISO 8859-1) character encoding, which limits the range of characters that can be used in these chunks. Encoders should
      prefer [iTXt](#11iTXt) to [tEXt](#11tEXt) and [zTXt](#11zTXt) chunks, in order to allow a wide range of characters without data loss. Encoders must convert characters
      that use local [legacy character encodings](https://www.w3.org/TR/i18n-glossary/#dfn-legacy-character-encodings) to the appropriate encoding when storing text.


When creating [iTXt](#11iTXt) chunks,
        encoders should follow [UTF-8 encode](https://encoding.spec.whatwg.org/#utf-8-encode) in [Encoding Standard](https://encoding.spec.whatwg.org/).


Encoders should discourage
      the creation of single lines of text longer than 79 Unicode [code points](https://www.w3.org/TR/i18n-glossary/#dfn-code-point), in order to facilitate easy reading. It is
      ...
      [image data](#dfn-image-data) (after the [IDAT](#11IDAT) chunks) can speed up image display in some situations,
      as the decoder will decode the [image data](#dfn-image-data) first. It is recommended that small text chunks, such as the image title,
      appear before the [IDAT](#11IDAT) chunks.
...
Examples of transmission errors are transmission in "text" or "ascii" mode, in which byte codes 13 and/or 10 may be added,
      ...
      absence of a [PLTE](#11PLTE) chunk before the first [IDAT](#11IDAT) chunk
      in an indexed image, or the presence of multiple [gAMA](#11gAMA) chunks. A PNG decoder should handle
      errors as follows:
...
- Detect errors as early as possible using the PNG signature bytes and CRCs on each chunk. Decoders should verify
        ...
        integrity if the next eight bytes begin an [IHDR](#11IHDR)chunk with the correct chunk length. A CRC should be checked before processing the chunk data. Sometimes this is impractical, for example when a streaming PNG decoder is processing a large[IDAT](#11IDAT)chunk. In this case the CRC should be checked when the end of the chunk is reached.
...
- known chunks, which necessarily includes all of the critical chunks defined in this specification ([IHDR](#11IHDR),[PLTE](#11PLTE),[IDAT](#11IDAT),[IEND](#11IEND))
...
PNG chunk types are marked "critical" or "ancillary" according to whether the chunks are critical for the purpose of
      extracting a viewable image (as with [IHDR](#11IHDR), [PLTE](#11PLTE), and
      [IDAT](#11IDAT)) or critical to understanding the datastream structure (as with [IEND](#11IEND)). This is a specific kind of criticality and one that is not necessarily relevant to every
      ...
      but should  [decode UTF-8 correctly](https://encoding.spec.whatwg.org/#utf-8-decode).
      Another decoder might consider the [tRNS](#11tRNS)
      and [gAMA](#11gAMA) chunks essential to its proper execution.
...
Syntax errors always involve known chunks because syntax errors in unknown chunks cannot be detected. The PNG decoder has
      ...
      example, most decoders can ignore an invalid [IEND](#11IEND) chunk; a text-extraction program can
      ignore the absence of [IDAT](#11IDAT); an image viewer cannot recover from an empty [PLTE](#11PLTE) chunk in an indexed image but it can ignore an invalid [PLTE](#11PLTE) chunk
      in a [truecolor](#3truecolour) image; and a program that extracts the alpha channel can ignore an invalid [gAMA](#11gAMA) chunk, but may consider the presence of two [tRNS](#11tRNS) chunks to be a fatal
...
When a fatal condition occurs, the decoder should fail immediately, signal an error to the user if appropriate, and
      optionally continue displaying any [image data](#dfn-image-data) already visible to the user (i.e. "fail gracefully"). The application as
...
Decoders that do not compute CRCs should interpret apparent syntax errors as indications of corruption (see also
      [13.2 Error checking](#13Error-checking)).

Errors in compressed chunks ( [IDAT](#11IDAT), [zTXt](#11zTXt),
      [iTXt](#11iTXt), [iCCP](#11iCCP)) could lead to buffer overruns.
      Implementors of [deflate](#dfn-deflate) decompressors should guard against this possibility.

APNG is designed to allow incremental display of frames before the entire [datastream](#5DataRep) has been
...
Decoders shall treat out-of-order APNG chunks as an error. APNG-aware [PNG editors](#dfn-png-editor) should restore them to correct
...
For known-length chunks, such as [IHDR](#11IHDR), decoders should treat an unexpected chunk
...
Unexpected values in fields of known chunks (for example, an unexpected compression method in the [IHDR](#11IHDR) chunk) shall be checked for and treated as errors. However, it is recommended that unexpected field values
      be treated as fatal errors only in **critical** chunks. An unexpected value in an ancillary chunk can be handled
...
Standard PNG images shall be compressed with compression method 0. The compression method field of the [IHDR](#11IHDR) chunk is provided for possible future standardization or proprietary variants. Decoders shall check
      this byte and report an error if it holds an unrecognized code. See [10. Compression](#10Compression) for details.
...
A PNG datastream is composed of a collection of explicitly typed chunks.
      ...
      Similarly there could be data after the [IEND](#11IEND) chunk
      which could contain anything, including malicious code.
      There is no known risk that such malicious code
      could be executed on the recipient's computer *as a result of decoding the PNG image*.
...
The [iTXt](#11iTXt), [tEXt](#11tEXt), and [zTXt](#11zTXt) chunks contain keywords and data that are meant to be displayed as plain text. The [iCCP](#11iCCP) and [sPLT](#11sPLT) chunks contain keywords that are meant to be displayed as
      ...
      decoders filter out control characters to avoid this risk; see [13.7 Text chunk processing](#13Text-chunk-processing).

For the [eXIf](#eXIf) chunk, the Exif Specification [[CIPA-DC-008](#bib-cipa-dc-008)] does not contain an express
...
A decoder that fails to check CRCs could be subject to data corruption. The only likely consequence of such
      corruption is incorrectly displayed pixels within the image. Worse things might happen if the CRC of the [IHDR](#11IHDR) chunk is not checked and the width or height fields are corrupted. See [13.2 Error checking](#13Error-checking).

A poorly written decoder might be subject to buffer overflow, because chunks can be extremely large, up to
      231-1 bytes long. But properly written decoders will handle large chunks without difficulty.
...
Non-square pixels can be represented (see [11.3.4.3 pHYs Physical pixel dimensions](#11pHYs)), but viewers are not required to account for them; a
...
When the [pHYs](#11pHYs) chunk has a unit specifier of 0 (unit is unknown), the behavior of a
      ...
      [pHYs](#11pHYs) chunk containing `(ppuX, ppuY, unit) = (2, 1, 0)` is equivalent to one
      containing `(1000, 500, 0)`; both are equally valid indications that the image pixels are twice as tall as they
      are wide.
...
```
Because other methods such as maintaining the image area are also reasonable, and because ignoring the [pHYs](#11pHYs) chunk is permissible, authors should not assume that all viewing applications will use this scaling
      method.
...
If practical, PNG decoders should have a way to display to the user all the [iTXt](#11iTXt),
      [tEXt](#11tEXt), and [zTXt](#11zTXt) chunks found in the datastream. Even
      if the decoder does not recognize a particular text keyword, the user might be able to understand it.

When processing [tEXt](#11tEXt) and [zTXt](#11zTXt) chunks, decoders
      ...
      non-Latin-1 characters (except for newline and perhaps TAB, FF, CR) encountered in a [tEXt](#11tEXt)
      or [zTXt](#11zTXt) chunk. Instead, they should be ignored or displayed in a visible notation such as
      "`\nnn`". See [13.3 Security considerations](#13Security-considerations).

When processing [iTXt](#11iTXt) chunks,
        decoders should follow [UTF-8 decode](https://encoding.spec.whatwg.org/#utf-8-decode) in [Encoding Standard](https://encoding.spec.whatwg.org/).
...
Decoders running on systems with a non-Latin-1 [legacy character encoding](https://www.w3.org/TR/i18n-glossary/#dfn-legacy-character-encodings) should remap character codes so that
      ...
      "`\nnn`". Characters should be only displayed if they are printable characters on the decoding system. Some byte
...
It is important to emphasize that [IDAT](#11IDAT) chunk boundaries have no semantic significance
      and can occur at any point in the compressed datastream. There is no required correlation between the structure of the
      [image data](#dfn-image-data) (for example, scanline boundaries) and [deflate](#dfn-deflate) block boundaries or [IDAT](#11IDAT) chunk boundaries. The complete [image data](#dfn-image-data) is represented by a single [zlib](#dfn-zlib) datastream that is
      stored in some number of [IDAT](#11IDAT) chunks; a decoder that assumes any more than this is
...
To reverse the effect of a filter, the decoder may need to use the decoded values of the prior pixel on the same line, the
      ...
      that at least one scanline's worth of [image data](#dfn-image-data) needs to be stored by the decoder at all times. Even though some
      ...
      the next scanline might use a filter type that refers to it. See [7.3 Filtering](#7Filtering).
...
When receiving images over slow transmission links, viewers can improve perceived performance by displaying interlaced
      ...
      be described by the following ISO C code [[ISO_9899](#bib-iso_9899)]:
...
```
The function `visit(row,column,height,width)` obtains the next transmitted pixel and paints a rectangle of the
...
If the viewer is merging the received image with a background image, it may be more convenient just to paint the received
      pixel positions (the `visit()` function sets only the pixel at the specified row and column, not the whole
...
To achieve PNG's goal of universal interchangeability, decoders shall accept all types of PNG image: [indexed-color](#3indexedColour),
      [truecolor](#3truecolour), and [greyscale](#dfn-greyscale). Viewers running on indexed-color display hardware need to be able to reduce
      [truecolor](#3truecolour) images to indexed-color for viewing. This process is called "color quantization".
...
Numerous implementations of color quantization are available. The PNG sample implementation, libpng ([ http://www.libpng.org/pub/png/libpng.html](http://www.libpng.org/pub/png/libpng.html)), includes code for
      the purpose.
...
```
 places. For example, to reduce 16-bit samples to 8-bit, the low-order byte can be discarded. In
      ...
      [gamma](#dfn-gamma) correction is being done, sample rescaling can be merged into the [gamma](#dfn-gamma) correction lookup table, as is
      illustrated in [13.13 Decoder gamma handling](#13Decoder-gamma-handling).)

If the decoder needs to scale samples up (for example, if the [frame buffer](#dfn-frame-buffer) has a greater sample depth than the PNG
      image), it should use linear scaling or left-bit-replication as described in [12.4 Sample depth scaling](#12Sample-depth-scaling).

When an [sBIT](#11sBIT) chunk is present, the reference [image data](#dfn-image-data) can be recovered by
      shifting right to the sample depth specified by [sBIT](#11sBIT). Note that linear scaling will not
      ...
      the [sBIT](#11sBIT) chunk; the stored image is a valid PNG datastream of the sample depth indicated
      by the [IHDR](#11IHDR) chunk; however, using [sBIT](#11sBIT) to recover the
      original samples before scaling them to suit the display often yields a more accurate display than ignoring [sBIT](#11sBIT).

When comparing pixel values to [tRNS](#11tRNS) chunk values to detect transparent pixels, the
...
For an image display program to produce correct tone reproduction, it is necessary to take into account the relationship
      between samples and display output, and the [transfer function](#dfn-transfer-function) of the display system. This can be done by
      calculating:
...
where `integer_sample` is the sample value from the datastream, `framebuf_sample` is the value to write
      into the [frame buffer](#dfn-frame-buffer), and `MAX_FRAMEBUF_SAMPLE` is the maximum value of a [frame buffer](#dfn-frame-buffer) sample (255
      ...
      display system's [transfer function](#dfn-transfer-function), and the fourth converts to an integer [frame buffer](#dfn-frame-buffer) sample. Zero raised to
      any positive power is zero.

A step could be inserted between the second and third to adjust `display_output` to account for the difference
...
The display [transfer function](#dfn-transfer-function) can typically be approximated by a power function with exponent
      `display_exponent`, in which case the second and third lines can be merged into:

`display_input = sample`
      1.0/(gamma * display_exponent) = sampledecoding_exponent
...
The [gamma value](#dfn-gamma-value) can be taken directly from the [gAMA](#11gAMA) chunk. Alternatively, an
      application may wish to allow the user to adjust the appearance of the displayed image by influencing the [gamma value](#dfn-gamma-value).
      For example, the user could manually set a parameter `user_exponent` which defaults to 1.0, and the application
      could set:
...
```
The user would set `user_exponent` greater than 1 to darken the mid-level tones, or less than 1 to lighten
      them.

A [gAMA](#11gAMA) chunk containing zero is meaningless but could appear by mistake. Decoders
      should ignore it, and editors may discard it and issue a warning to the user.

It is **not** necessary to perform a transcendental mathematical computation for every pixel. Instead, a
...
If floating-point calculations are not possible, [gamma](#dfn-gamma) correction tables can be computed using integer arithmetic
      and a precomputed table of logarithms. Example code appears in [[PNG-EXTENSIONS](#bib-png-extensions)].

When the incoming image has unknown [gamma value](#dfn-gamma-value) ([gAMA](#11gAMA), [sRGB](#srgb-standard-colour-space), and [iCCP](#11iCCP) all absent), standalone image viewers
      should choose a likely default [gamma value](#dfn-gamma-value), but allow the user to select a new one if the result proves too dark or
      too light. The default [gamma value](#dfn-gamma-value) may depend on other knowledge about the image, for example whether it came from the
      Internet or from the local system. For consistency, viewers for document formats such as HTML, or vector graphics such as
      SVG, should treat embedded or linked PNG images with unknown [gamma value](#dfn-gamma-value) in the same way that they treat other
      untagged images.

In practice, it is often difficult to determine what value of display exponent should be used. In systems with no built-in
      [gamma](#dfn-gamma) correction, the display exponent is determined entirely by the CRT. A display exponent of 2.2 should be
      used unless detailed calibration measurements are available for the particular CRT used.

Many modern [frame buffers](#dfn-frame-buffer) have lookup tables that are used to perform [gamma](#dfn-gamma) correction, and on these systems
      ...
      should go into the lookup table, so interpretation of the system [gamma value](#dfn-gamma-value) is system-dependent.
...
The response of real displays is actually more complex than can be described by a single number (the display exponent). If
      ...
      the computation above can be replaced by a lookup in these measurements, to find the actual [frame buffer](#dfn-frame-buffer) value that
      most nearly gives the desired brightness.

See [C. Gamma and chromaticity](#C-GammaAppendix) for references to color issues.

In many cases, the [image data](#dfn-image-data) in PNG datastreams will be treated as device-dependent RGB values and displayed
      without modification (except for appropriate [gamma](#dfn-gamma) correction). This provides the fastest display of PNG images. But
      ...
      not be exactly the same as those seen by the original author, particularly for darker or near-neutral colors. The [cHRM](#11cHRM) chunk provides information that allows closer color matching than that provided by
      [gamma](#dfn-gamma) correction alone.

The [cHRM](#11cHRM) data can be used to transform the [image data](#dfn-image-data) from RGB to XYZ and
      ...
      space, could be used for display or written into a [PLTE](#11PLTE) chunk.

Decoders that are part of image processing applications might also transform [image data](#dfn-image-data) into CIE LAB space for
      analysis.

In applications where color fidelity is critical, such as product design, scientific visualization, medicine,
      architecture, or advertising, PNG decoders can transform the [image data](#dfn-image-data) from source RGB to the display RGB space of
...
Decoders running on platforms that have a Color Management System (CMS) can pass the [image data](#dfn-image-data), [gAMA](#11gAMA), and [cHRM](#11cHRM) values to the CMS for display or further
      processing.

PNG decoders that provide color printing facilities can use the facilities in Level 2 PostScript to specify [image
      data](#dfn-image-data) in calibrated RGB space or in a device-independent color space such as XYZ. This will provide better color
      fidelity than a simple RGB to CMYK conversion. The PostScript Language Reference manual [[PostScript](#bib-postscript)] gives examples. Such
      decoders are responsible for implementing gamut mapping between source RGB (specified in the [cHRM](#11cHRM) chunk) and the target printer. The PostScript interpreter is then responsible for producing the required
      colors.

PNG decoders can use the [cHRM](#11cHRM) data to calculate an accurate greyscale representation
      of a color image. Conversion from RGB to grey is simply a case of calculating the Y (luminance) component of XYZ, which is a
      weighted sum of R, G, and B values. The weights depend upon the monitor type, i.e. the values in the [cHRM](#11cHRM) chunk. PNG decoders may wish to do this for PNG datastreams with no [cHRM](#11cHRM) chunk. In this case, a reasonable default would be the CCIR 709 primaries [[ITU-R-BT.709](#bib-itu-r-bt.709)]. The original
      NTSC primaries should **not** be used unless the PNG image really was color-balanced for such a monitor.

The background color given by the [bKGD](#11bKGD) chunk will typically be used to fill unused
      screen space around the image, as well as any transparent pixels within the image. (Thus, [bKGD](#11bKGD) is valid and useful even when the image does not use transparency.) If no [bKGD](#11bKGD) chunk is present, the viewer will need to decide upon a suitable background color. When no other
...
Viewers that have a specific background against which to present the image (such as web browsers) should ignore the
      [bKGD](#11bKGD) chunk, in effect overriding [bKGD](#11bKGD) with their
      preferred background color or background image.

The background color given by the [bKGD](#11bKGD) chunk is not to be considered transparent,
      even if it happens to match the color given by the [tRNS](#11tRNS) chunk (or, in the case of an
      [indexed-color](#3indexedColour) image, refers to a palette index that is marked as transparent by the [tRNS](#11tRNS)
      chunk). Otherwise one would have to imagine something "behind the background" to [composite](#dfn-composited) against. The background
      color is either used as background or ignored; it is not an intermediate layer between the PNG image and some other
      background.

Indeed, it will be common that the [bKGD](#11bKGD) and [tRNS](#11tRNS)
...
The alpha channel can be used to [composite](#dfn-composited) a foreground image against a background image. The PNG datastream
      defines the foreground image and the transparency mask, but not the background image. PNG decoders are **not**
...
`output = alpha * foreground + (1-alpha) * background`where alpha and the input and output sample values are expressed as fractions in the range 0 to 1. This computation should
      be performed with intensity samples (not [gamma](#dfn-gamma)-encoded samples). For color images, the computation is done separately
...
The following code illustrates the general case of compositing a foreground image against a background image. It assumes
      that the original pixel data are available for the background image, and that output is to a [frame buffer](#dfn-frame-buffer) for display.
      Other variants are possible; see the comments below the code. The code allows the sample depths and [gamma values](#dfn-gamma-value) of
...
- If output is to another PNG datastream instead of a [frame buffer](#dfn-frame-buffer), lines 21, 22, 33, and 34 should be changed along the following lines`/* * Gamma encode for storage in output datastream. * Convert to integer sample value. */ gamout = pow(comppix, outfile_gamma); outpix[i] = (int) (gamout * out_maxsample + 0.5);`
- If the sample depths of the output file, foreground file, and background file are all the same, and the three [gamma values](#dfn-gamma-value)also match, then the no-compositing code in lines 14-23 reduces to copying pixel values from the input file to the output file if alpha is one, or copying pixel values from background to output file if alpha is zero. Since alpha is typically either zero or one for the vast majority of pixels in an image, this is a significant saving. No[gamma](#dfn-gamma)computations are needed for most pixels.
- When the sample depths and [gamma values](#dfn-gamma-value)all match, it may appear attractive to skip the[gamma](#dfn-gamma)decoding and encoding (lines 28-31, 33-34) and just perform line 32 using[gamma](#dfn-gamma)-encoded sample values. Although this does not have too bad an effect on image quality, the time savings are small if alpha values of zero and one are treated as special cases as recommended here.
...
- Note that lines 18-22 are performing exactly the same [gamma](#dfn-gamma)computation that is done when no alpha channel is present. If the no-alpha case is handled with a lookup table, the same lookup table can be used here. Lines 28-31 and 33-34 can also be done with (different) lookup tables.
...
For viewers running on indexed-color hardware attempting to display a [truecolor](#3truecolour) image, or an indexed-color image
      whose palette is too large for the [frame buffer](#dfn-frame-buffer), the encoder may have provided one or more suggested palettes in
      [sPLT](#11sPLT) chunks. If one of these is found to be suitable, based on size and perhaps name, the
      PNG decoder can use that palette. Suggested palettes with a sample depth different from what the decoder needs can be
      converted using sample depth rescaling (see [13.12 Sample depth rescaling](#13Sample-depth-rescaling)).

When the background is a solid color, the viewer should [composite](#dfn-composited) the image and the suggested palette against that
...
For [truecolor](#3truecolour) images, a suggested palette might also be provided in a [PLTE](#11PLTE)
      chunk. If the image has a [tRNS](#11tRNS) chunk and the background is a solid color, the viewer
      ...
      the [tRNS](#11tRNS) color should be replaced with the desired background color; or alternatively a
      palette entry for the background color can be added, if the viewer can handle more colors than there are [PLTE](#11PLTE) entries.

For images of [color type](#3colourType) 6 ([truecolor with alpha](#dfn-truecolor-with-alpha)), any [PLTE](#11PLTE) chunk
      should have been designed for display of the image against a uniform background of the color specified by the [bKGD](#11bKGD) chunk. Viewers should probably ignore the palette if they intend to use a different
      background, or if the [bKGD](#11bKGD) chunk is missing. Viewers can use a suggested palette for
      display against a different background than it was intended for, but the results may not be very good.

If the viewer presents a transparent [truecolor](#3truecolour) image against a background that is more complex than a uniform
      color, it is unlikely that the suggested palette will be optimal for the [composite](#dfn-composited) image. In this case it is best to
      perform a [truecolor](#3truecolour) compositing step on the [truecolor](#3truecolour) PNG image and background image, then color-quantize
      the resulting image.

In [truecolor](#3truecolour) PNG datastreams, if both [PLTE](#11PLTE) and [sPLT](#11sPLT) chunks appear, the PNG decoder may choose from among the palettes suggested by both, bearing in mind the
      different transparency semantics described above.

The frequencies in the [sPLT](#11sPLT) and [hIST](#11hIST) chunks are
      ...
      be used as the input data, thus avoiding a scan of the [image data](#dfn-image-data) in the [IDAT](#11IDAT)
      chunks.

If no suggested palette is provided, a decoder can develop its own, at the cost of an extra pass over the [image
      data](#dfn-image-data) in the [IDAT](#11IDAT) chunks. Alternatively, a default palette (probably a color cube)
      can be used.

See also [12.5 Suggested palettes](#12Suggested-palettes).

Authors are encouraged to look existing chunk types in both this specification and [[PNG-EXTENSIONS](#bib-png-extensions)] before considering
      introducing a new chunk types. The chunk types at [[PNG-EXTENSIONS](#bib-png-extensions)] are expected to be less widely supported than those
      defined in this specification.

Two examples of [PNG editors](#dfn-png-editor) are a program that adds or modifies text chunks, and a program that adds a suggested
      palette to a [truecolor](#3truecolour) PNG datastream. Ordinary image editors are not [PNG editors](#dfn-png-editor) because they usually discard
...
To allow new chunk types to be added to PNG, it is necessary to establish rules about the ordering requirements for all
      chunk types. Otherwise a [PNG editor](#dfn-png-editor) does not know what to do when it encounters an unknown chunk.

EXAMPLE Consider a hypothetical new ancillary chunk type that is safe-to-copy and is required to appear after [PLTE](#11PLTE) if [PLTE](#11PLTE) is present. If a program attempts to add a
      [PLTE](#11PLTE) chunk and does not recognize the new chunk, it may insert the [PLTE](#11PLTE) chunk in the wrong place, namely after the new chunk. Such problems could be prevented by requiring [PNG
      editors](#dfn-png-editor) to discard all unknown chunks, but that is a very unattractive solution. Instead, PNG requires ancillary chunks
...
To prevent this type of problem while allowing for future extension, constraints are placed on both the behavior of
      [PNG editors](#dfn-png-editor) and the allowed ordering requirements for chunks. The safe-to-copy bit defines the proper handling of
      unrecognized chunks in a datastream that is being modified.

- If a chunk's safe-to-copy bit is 1, the chunk may be copied to a modified PNG datastream whether or not the [PNG editor](#dfn-png-editor)recognizes the chunk type, and regardless of the extent of the datastream modifications.
...
- When copying an unknown **safe-to-copy**ancillary chunk, a[PNG editor](#dfn-png-editor)shall not move the chunk from before[IDAT](#11IDAT)to after[IDAT](#11IDAT)or vice versa. (This is well defined because[IDAT](#11IDAT)is always present.) Any other reordering is permitted.
...
[PNG editors](#dfn-png-editor) that do not change the [image data](#dfn-image-data) should not change the [tIME](#11tIME)
      chunk. The Creation Time keyword in the [tEXt](#11tEXt), [zTXt](#11zTXt),
      and [iTXt](#11iTXt) chunks may be used for a user-supplied time.

Critical chunks may have arbitrary ordering requirements, because [PNG editors](#dfn-png-editor) are required to terminate if they
        encounter unknown critical chunks. For example [IHDR](#11IHDR) has the specific ordering rule that
...
- Safe-to-copy chunks may have ordering requirements relative to [IDAT](#11IDAT).

The actual ordering rules for any particular ancillary chunk type may be weaker. See for example the ordering rules for
        the standard ancillary chunk types in [5.6 Chunk ordering](#5ChunkOrdering).
...
EXAMPLE It is unsafe to assume that a particular private ancillary chunk occurs immediately before [IEND](#11IEND). Even if it is always written in that position by a particular application, a [PNG editor](#dfn-png-editor)
        might have inserted some other ancillary chunk after it. But it is safe to assume that the chunk will remain somewhere
        between [IDAT](#11IDAT) and [IEND](#11IEND).

As well as sections marked as non-normative, all authoring guidelines, diagrams, examples, and notes in this specification are non-normative. Everything else in this specification is normative.

        The key words *MAY*, *MUST*, *SHALL*, *SHOULD*, and *SHOULD NOT* in this document
...
This clause addresses the PNG datastream and implementation requirements including the range of allowable differences
        for PNG encoders, PNG decoders, and [PNG editors](#dfn-png-editor). This clause does not directly address the environmental,
...
- The PNG datastream contains a PNG signature as the first content (see [5.2 PNG signature](#5PNG-file-signature)).
- With respect to the chunk types defined in this International Standard:
- No chunks or other content follow the [IEND](#11IEND)chunk.
...
-
            [Private field values](#dfn-private-field-values)are used when encoding experimental or private definitions of values for any of the method or type fields.
...
- Unknown chunk types are handled as described in [5.4 Chunk naming conventions](#5Chunk-naming-conventions). An unknown chunk type is**not**treated as an error unless it is a critical chunk.
- Unexpected values in fields of known chunks (for example, an unexpected compression method in the [IHDR](#11IHDR)chunk) are treated as errors.
- All types of PNG images (indexed-color, [truecolor](#3truecolour),[greyscale](#dfn-greyscale),[truecolor with alpha](#dfn-truecolor-with-alpha), and[greyscale with alpha](#dfn-greyscale-with-alpha)) are processed. For example, decoders which are part of viewers running on indexed-color display hardware shall reduce[truecolor](#3truecolour)images to indexed format for viewing.
...
- All valid combinations of bit depth and [color type](#3colourType)as defined in[11.2.1 IHDR Image header](#11IHDR)are supported.
- An error is reported if an unrecognized value is encountered in the bit depth, [color type](#3colourType), compression method,[filter method](#3filter), or interlace method bytes of the[IHDR](#11IHDR)chunk.
- When processing 16-bit [greyscale](#dfn-greyscale)or[truecolor](#3truecolour)data in the[tRNS](#11tRNS)chunk, both bytes of the sample values are evaluated to determine whether a pixel is transparent.
- When processing an image compressed by compression method 0, the decoder assumes no more than that the complete
          [image data](#dfn-image-data)is represented by a single compressed datastream that is stored in some number of[IDAT](#11IDAT)chunks.
...
- It preserves the ordering of the chunks presented within the rules in [5.6 Chunk ordering](#5ChunkOrdering).
...
- Unless the user specifically permits lossy operations or the editor issues a warning, it preserves all information
          ...
          preserved if it contains only zero and maximum values. Operations such as changing the [color type](#3colourType)or rearranging the palette in an[indexed-color](#3indexedColour)datastream are permitted provided that the new datastream losslessly represents the same reference image.

This updates the existing image/png Internet Media type, under the image
      top level type. This appendix is in conformance with [BCP 13](https://www.rfc-editor.org/info/bcp13) and [W3CRegMedia](https://www.w3.org/2020/01/registering-mediatypes.html).
...
- General Comments:
-
          This registration updates the earlier one: - The old one points to an expired Internet Draft. This updated registration points to a W3C Recommendation.
...
A [gamma value](#dfn-gamma-value) is a numerical parameter used to describe approximations to certain non-linear [transfer
    functions](#dfn-transfer-function) encountered in image capture and reproduction. The [gamma value](#dfn-gamma-value) is the exponent in a power law function.
...
For the purposes of this specification, it is convenient to consider five places in a general image pipeline at which
    non-linear [transfer functions](#dfn-transfer-function) may occur and which may be modelled by power laws. The characteristic exponent associated
...
| LUT_exponent | the exponent of the [transfer function](#dfn-transfer-function)applied between the[frame buffer](#dfn-frame-buffer)and the display device (typically
...
It is convenient to define some additional entities that describe some composite [transfer functions](#dfn-transfer-function), or combinations
    of stages.

| display_exponent | exponent of the [transfer function](#dfn-transfer-function)applied between the[frame buffer](#dfn-frame-buffer)and the display surface of the display
...
The PNG [gAMA](#11gAMA) chunk is used to record the [gamma value](#dfn-gamma-value). This information may be
...
The following sample code — which is informative — represents a practical implementation of the CRC (Cyclic
    Redundancy Check) employed in PNG chunks. (See also ISO 3309 [[ISO-3309](#bib-iso-3309)] or ITU-T V.42 [[ITU-T-V.42](#bib-itu-t-v.42)] for a formal
    specification.)

The sample code is in the ISO C [[ISO_9899](#bib-iso_9899)] programming language. The hints in [Table 31](#D-tabled1) may help non-C
...
| `++` | " `n++`" increments the variablen. In "for" loops, it is applied after the variable is
        tested. |
| `0xNNN` | `0x`introduces a hexadecimal (base 16) constant. Suffix`L`indicates a long value (at least 32
        bits). |
...
There is a World Wide Web site for PNG at [ http://www.libpng.org/pub/png/](http://www.libpng.org/pub/png/). This page is a central location for current
      information about PNG and PNG-related tools.

Additional documentation and portable C code for [deflate](#dfn-deflate), and an optimized implementation of the CRC
      algorithm are available from the zlib web site, [ https://www.zlib.net/](https://www.zlib.net/).

A sample implementation in portable C, **libpng**, is available at [ http://www.libpng.org/pub/png/libpng.html](http://www.libpng.org/pub/png/libpng.html). Sample viewer and
      encoder applications of libpng are available at

`http://www.libpng.org/pub/png/book/sources.html`*This section is non-normative.*
...
- Specified interoperable handling of extra sample bits, beyond the specified bit depth,
        in [tRNS](#11tRNS)and[bKGD](#11bKGD)chunks.
...
-
        The three previously defined, but unofficial, chunks for Animated PNG (APNG) have been added: This brings the PNG specification into alignment with widely deployed industry practice.
-
