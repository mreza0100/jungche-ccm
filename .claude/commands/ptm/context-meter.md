...
- `.claude/commands/**/*.md`, nested command dirs included (`ptm/`, `wave/`, `quality/`, `audit/`, `p/`, `h/`): over 35 KB
...
The size limits are `/ptm`'s (§ Hard thresholds, § Critical invariants); this command only measures against them.
...
- Report only — never edit. Trimming a prompt file routes through `/ptm`, which loads `/quality:prompt` and verifies consistency. Surface the savings; the user approves the cut.
