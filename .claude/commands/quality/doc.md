...
**When to load:** before hand-editing or restructuring anything under `docs/`, an engine's spec (`pfm/*.md`, `dreamer/SPEC-*.md`, `engines/**/design.md`), or `docs/{BLUEPRINT,SETUP,PLACEHOLDERS}.md`. These are read by agents with `Read` and `grep`, not by a human in a rendered viewer.
