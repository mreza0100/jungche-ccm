...
3. **Dispatch** — instructions travel through the orchestrating seat, never to the builder directly; git writes only via `gitter`; guarded files (`.claude/**`, any `CLAUDE.md`, `blueprint/**`) via `/pcm`.
...
7. **Failure-mode sweep** — the recurring diseases, probed by name:
   ...
   - **Structure drift** — every wave closes by a gitter commit: markdown-only waves land on `main` directly, code waves build in the train's fenced worktree and merge at gated close (QA pass → orchestrator review → gitter). A substitute gate (a tracer or conformance pass standing in for the walker), a git WRITE by any seat but gitter, or a reorder of an approved train without a user ruling in the ledger — each a finding naming the artifact.
