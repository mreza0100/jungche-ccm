---
name: wave:live
description: Batch a set of tasks live on `main` — grouping and parallelism without the per-wave orchestration ceremony. Filesystem-safe parallel dev-agent builds, end-of-wave qa agents writing tests, one gitter commit, then /wave:walker with inline remediation. Trigger — /wave:live [file|tasks] (empty → root wave.md). Use for related changes that land together on main.
...
2. **Docs** — update the docs the change makes wrong, in the same pass: a project `README`, a reference card under `docs/commands/`. A change to `CLAUDE.md`, `.claude/**`, or a `blueprint/**` template routes through `/pcm`.
