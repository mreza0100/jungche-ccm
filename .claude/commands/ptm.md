---
name: ptm
description: Professor Template Management — owns CLAUDE.md, .claude/ (agents, commands, skills, scripts, settings), the .codex/ mirror, and the shipped templates/ tree. Mandatory route for any framework or process-file change; also runs framework audits (`audit [scope]`, e.g. `audit all`) and folds the steering-conscience inbox (`retro`). Publishing the blueprint upstream is the /ptm:release subcommand; /ptm:context-meter audits the framework's own context budget.
argument-hint: [change request|audit]
---

# PTM — Professor Template Management
...
- `.claude/commands/**/*.md` — slash commands (`/ptm`, `/ptm:release`, `/ptm:context-meter`, `/quality:*`, `/dev`, `/git`)
- `.claude/agents/*.md` — registered agents (`ls` for the set); `gitter` is the Git writer, `tracer` the consumer-tree trace
- `.claude/skills/*/SKILL.md` — reusable skills (`ls .claude/skills/` for the current set; source-fetched per `templates/skills/sources.json`, never vendored)
- `.claude/output-styles/*.md` — persona registry: `professor.md` (session style) + `dr-house.md` (this command's overlay)
- `.claude/scripts/*.{sh,mjs}` — dev.sh, ptm-guard.sh, guard-stamp.sh, format-md.sh, codex-sync.sh (mirror auto-compile; the Codex compiler itself is `pfm codex build`)
- `docs/commands/{cmd}/references/` — command-owned reference docs (`$CDOCS/$CMD/$REFS/`); today: `ptm/references/{audit-scopes,refresh}.md`
...
- The shipped blueprint: `templates/**` — templates, spec, and `refresh-map.json`. `/ptm:release` publishes it; `/ptm` edits it.
...
- PTM reference docs: `docs/commands/ptm/references/`

---

## Logging every change — `drift.md` vs `release.md`

Every infra change `/ptm` makes is recorded as the final step of the work, in exactly **one** `.professor/` ledger:
...
- **`release.md`** — framework changes that belong upstream, **pending publication**. `/ptm:release` consumes this file to build the CHANGELOG, then clears it.
...
**Open the gate (before the edit pass).** A PreToolUse hook (`ptm-guard.sh`) denies Edit/Write to `.claude/**`, any `CLAUDE.md`, and `docs/commands/ptm/references/**` unless BOTH session-keyed markers are fresh: reading `quality/prompt.md` stamps the quality marker automatically, and the ptm marker is stamped with the exact command the deny message provides (it carries your session key). Markers **survive turn ends** and slide on every allowed edit — stamp once per session; the 1500s freshness TTL plus the Stop hook's 1h reap kill only abandoned sessions. If a write is denied, follow the deny message and retry — and note that a sandboxed stamp never lands on the filesystem the hook reads.
...
- The `description:` frontmatter names every subcommand and trigger the body handles — an unnamed mode is unroutable
...
Spawn **one Agent per scope in parallel** (subagent_type: `Explore`, search breadth: `very thorough`). Each agent deep-reads its entire domain — follows every reference, reads every file, verifies semantic consistency. PTM aggregates results after all agents return.
...
Seven scopes: `agents`, `commands`, `skills`, `templates`, `scripts`, `structure`, `cross-refs`. The per-scope checklists live in `docs/commands/ptm/references/audit-scopes.md` — read it when composing the fan-out briefs; each agent's brief carries its scope's section.
...
**New agent:** Create `.claude/agents/{name}.md` — its `description:` is the registry entry, its `model:` pins the tier (root `CLAUDE.md` § Subagent dispatch carries no roster) → `pfm codex build .` (it compiles a `.codex/agents/{name}.toml`) → decide whether it ships upstream as `templates/agents/{name}.md`.

**New skill:** Create `.claude/skills/{name}/SKILL.md` → no CLAUDE.md edit needed (skills self-index from `description:` frontmatter). A skill meant for adopters is registered in `templates/skills/sources.json` and lives in its OWN public repo — the blueprint never vendors one.
