---
name: pfm
description: Professor Framework Management — owns CLAUDE.md, .claude/ (agents, commands, skills, scripts, settings), the .codex/ mirror, and the shipped templates/ tree. Mandatory route for any framework or process-file change; also runs framework audits (`audit [scope]`, e.g. `audit all`) and folds the steering-conscience inbox (`retro`). Publishing the blueprint upstream is the /pfm:release subcommand; the global /context-meter audits the framework's own context budget.
