...
1. **SCOUT + INVENTORY — at most 3 Bash calls, batched.** Compound every command (`cmd1; cmd2; …`):
   ...
   `blueprint/templates/**`, and every `*.md` under them are IN scope, never excluded.** A trace that
   skips `blueprint/templates/` has skipped the product. Enumerate ≤6 THREADS (default 4 — every extra
...
| --- | --- |
| placeholder token → filled value | `blueprint/PLACEHOLDERS.md` (the registry) → every `blueprint/templates/**` occurrence → `blueprint/SETUP.md`'s write step for that file |
