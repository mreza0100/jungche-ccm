...
1. **SCOUT + INVENTORY — at most 3 Bash calls, batched.** Compound every command (`cmd1; cmd2; …`):
   ...
   `templates/**`, and every `*.md` under them are IN scope, never excluded.** A trace that
   skips `templates/` has skipped the product. Enumerate ≤6 THREADS (default 4 — every extra
