...
**Repository:** one git repo holding three projects — `blueprint/` (the shipped framework), `pfm/` (Go, including the memory organ), and `engines/wave-walker/engine/` (JS). No submodules. `main` is the published branch; code waves build under `.worktrees/{train}/`.
...
- `<scope>`: `blueprint`, `pfm`, `walker`, `professor` (the install itself), or omitted for repo-wide chores.
