...
- [v0.49.0](releases/v0.49.0.md) — the host-swap fleet is genuinely dual-runtime: one POSIX seam (`cc-portable.sh`) behind every GNU/BSD split, and eight Linux-only assumptions fixed that gave macOS empty answers instead of errors — `find -printf` (the whole `cc-ls` chat index), `flock` (`cc-name-sync` exited before doing any work, status 0, every run), `setsid` (`/bb`, `/swap` and `--then` never detached), `/proc/<pid>/environ` (every background agent defaulted to account 1, sending takeovers to the wrong subscription), plus `stat`/`date`/`ps`/`free`/`ss`/`timeout`; `_cc_label` stops calling the primary account "(not logged in)" on every platform; the four fixtures stop blaming innocent code — 106 assertions, 0 failures on macOS
