...
func runHeadless(args []string, stdout, stderr io.Writer) int {
	...
	case "help", "-h", "--help":
		...
		return 0
	default:
		fmt.Fprintf(stderr, "cc-fleet headless: unknown command %q\n", verb)
		printHeadlessUsage(stderr)
		return 2
	}
}
...
func printHeadlessUsage(w io.Writer) {
	fmt.Fprintln(w, "usage: cc-fleet headless <command> [options]")
	fmt.Fprintln(w)
...
func headlessTarget(
	...
	if err != nil {
		fmt.Fprintf(stderr, "cc-fleet headless: %v\n", err)
	...
	if !found {
		...
		fmt.Fprintf(stderr, "cc-fleet headless: no chat named %q\n", name)
...
func runHeadlessStatus(args []string, stdout, stderr io.Writer) int {
	flags := newFlagSet(
		"headless status",
		"usage: cc-fleet headless status <name> [--json]",
		stderr,
	)
	...
	if err != nil {
		fmt.Fprintf(stderr, "cc-fleet headless status: %v\n", err)
		return 1
	}
	if *asJSON {
...
func runHeadlessTranscript(args []string, stdout, stderr io.Writer) int {
	flags := newFlagSet(
		"headless transcript",
		"usage: cc-fleet headless transcript <name> [--tail N] [--condensed] [--json]",
		stderr,
	)
	...
	if chat.Path == "" {
		fmt.Fprintf(stderr, "cc-fleet headless transcript: %q has not written a transcript yet\n", chat.Name)
		return codeDeadChat
	}
	entries, truncated, err := transcript.Tail(ctx, chat.Path, chat.Engine, *tail, 0)
	if err != nil {
		fmt.Fprintf(stderr, "cc-fleet headless transcript: %v\n", err)
		return 1
	}
	switch {
	case *asJSON:
...
func runHeadlessLast(args []string, stdout, stderr io.Writer) int {
	flags := newFlagSet(
		"headless last",
		"usage: cc-fleet headless last <name>",
		stderr,
	)
	names, code, ok := parseFlagsAnywhere(flags, args)
	if !ok {
	...
	if chat.Path == "" {
		fmt.Fprintf(stderr, "cc-fleet headless last: %q has not written a transcript yet\n", chat.Name)
		return codeDeadChat
	}
	...
	if err != nil {
		fmt.Fprintf(stderr, "cc-fleet headless last: %v\n", err)
		return 1
	}
	entry, found := transcript.Last(entries, transcript.RoleAssistant)
	if !found {
		fmt.Fprintf(stderr, "cc-fleet headless last: %q has not answered yet\n", chat.Name)
		return codeDeadChat
	}
...
func runHeadlessStream(args []string, stdout, stderr io.Writer) int {
	flags := newFlagSet(
		"headless stream",
		"usage: cc-fleet headless stream <name> [--filter REGEX] [--margin N] "+
	...
	if *filter != "" {
		...
		if err != nil {
			fmt.Fprintf(stderr, "cc-fleet headless stream: bad --filter: %v\n", err)
			return 2
		}
		pattern = compiled
	}
	...
	if chat.Path == "" {
		fmt.Fprintf(stderr, "cc-fleet headless stream: %q has not written a transcript yet\n", chat.Name)
		return codeDeadChat
	}
	name := chat.Name
	...
	if err != nil {
		if err == headless.ErrChatGone {
			fmt.Fprintf(stderr, "cc-fleet headless stream: %s is gone\n", name)
			return codeDeadChat
		}
		fmt.Fprintf(stderr, "cc-fleet headless stream: %v\n", err)
		return 1
	}
	return 0
}
...
func runHeadlessInject(args []string, stdout, stderr io.Writer) int {
	flags := newFlagSet(
		"headless inject",
		"usage: cc-fleet headless inject <name> <message>",
		stderr,
	)
	...
	if !chat.Live {
		fmt.Fprintf(stderr, "cc-fleet headless inject: %q is not running\n", chat.Name)
		return codeDeadChat
	}
...
func runHeadlessWatch(args []string, stdout, stderr io.Writer) int {
	flags := newFlagSet(
		"headless watch",
		"usage: cc-fleet headless watch <name> [--idle-after SECS] "+
	...
	if err != nil {
		fmt.Fprintf(stderr, "cc-fleet headless watch: %v\n", err)
		return 1
	}
	if !status.Alive() {
		return codeDeadChat
...
func hookRunner(command string, stderr io.Writer) func(headless.Status) error {
	...
	return func(status headless.Status) error {
		...
		if err := process.Run(); err != nil {
			fmt.Fprintf(stderr, "cc-fleet headless watch: hook failed: %v\n", err)
		}
		return nil
	}
}
...
func runHeadlessLS(args []string, stdout, stderr io.Writer) int {
	flags := newFlagSet(
		"headless ls",
		"usage: cc-fleet headless ls [--json]",
		stderr,
	)
	...
	if err != nil {
		fmt.Fprintf(stderr, "cc-fleet headless ls: %v\n", err)
		return 1
	}
	defer database.Close()
	scan, err := scanFleet(
