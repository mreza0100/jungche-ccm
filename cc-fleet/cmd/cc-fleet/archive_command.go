...
import (
	...
	"hostops/cc-fleet/internal/archive"
...
func runArchive(args []string, stdout, stderr io.Writer) int {
	flags := newFlagSet(
		"archive",
		"usage: cc-fleet archive [--apply] [--subagents [--older-than DAYS]] [--restore id]",
		stderr,
	)
	...
	if err != nil {
		fmt.Fprintf(stderr, "cc-fleet archive: %v\n", err)
		return 1
	}
	if *restore != "" {
...
func printArchiveReport(
	...
	fmt.Fprintln(stdout, "put one back with: cc-fleet archive --restore <id>")
}
