...
func runHeadlessAsk(args []string, stdout, stderr io.Writer) int {
	flags := newFlagSet(
		"headless ask",
		"usage: cc-fleet headless ask [--timeout SECS] [--settle SECS] [--now] "+
	...
	if !chat.Live {
		fmt.Fprintf(stderr, "cc-fleet headless ask: %q is not running\n", chat.Name)
		return codeDeadChat
	}
	...
	if err != nil {
		fmt.Fprintf(stderr, "cc-fleet headless ask: %v\n", err)
		return 1
	}
	...
	if result.Code != 0 {
		fmt.Fprintf(stderr, "cc-fleet headless ask: %s\n", result.Message)
	...
	if budget > 0 && remaining <= 0 {
		fmt.Fprintf(
			stderr,
			"cc-fleet headless ask: %s took the whole %ds to become free — "+
...
//
// The answer goes to stdout ALONE — `$(cc-fleet headless ask …)` is the reply
...
func awaitAnswer(
	...
	if turn.Superseded {
		fmt.Fprintf(
			stderr,
			"cc-fleet headless %s: another message reached %s while this one was "+
	...
	case errors.Is(err, headless.ErrAwaitTimeout):
		fmt.Fprintf(
			stderr,
			"cc-fleet headless %s: %s is still working after %ds — the message "+
	...
	case errors.Is(err, headless.ErrChatGone):
		if turn.Answer != "" {
			...
			fmt.Fprintf(stderr, "cc-fleet headless %s: %s answered and is now gone\n", verb, name)
			return 0
		}
		fmt.Fprintf(stderr, "cc-fleet headless %s: %s is gone — it never answered\n", verb, name)
		return codeDeadChat
	default:
		fmt.Fprintf(stderr, "cc-fleet headless %s: %v\n", verb, err)
		return 1
	}
}
