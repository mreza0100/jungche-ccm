...
import (
	...
	"hostops/cc-fleet/internal/dream"
)
...
func productionDreamRuntime() dreamCommandRuntime {
	return dreamCommandRuntime{
		night: func(
			...
			return fmt.Sprintf(
				"dreamer-night: signed apply command: cc-fleet dream apply --repo %s --agent %s %s\n",
				options.RepoRoot,
				options.AgentType,
				result.Stage.Root,
			), nil
...
func runDreamWith(
	...
	case "help", "-h", "--help":
		...
		return 0
	default:
		fmt.Fprintf(stderr, "cc-fleet dream: unknown command %q\n", args[0])
		printDreamUsage(stderr)
		return 2
	}
}
...
func runDreamNight(
	...
	flags := newFlagSet(
		"dream night",
		"usage: cc-fleet dream night [--repo ROOT] [--agent TYPE] [--bootstrap-count N | --corpus-file FILE]",
		stderr,
	)
...
func runDreamApply(
	...
	flags := newFlagSet(
		"dream apply",
		"usage: cc-fleet dream apply [--repo ROOT] [--agent TYPE] STAGE",
		stderr,
	)
	repo := flags.String("repo", defaultDreamRepo, "repository root")
...
func runDreamInspect(
	...
	flags := newFlagSet(
		"dream inspect",
		"usage: cc-fleet dream inspect [--repo ROOT] [--agent TYPE]",
		stderr,
	)
	repo := flags.String("repo", defaultDreamRepo, "repository root")
...
func runDreamMorning(
	...
	flags := newFlagSet("dream morning", "usage: cc-fleet dream morning", stderr)
	if code, ok := parseFlags(flags, args); !ok {
		return code
	}
	if flags.NArg() != 0 {
	...
	if err != nil {
		fmt.Fprintf(stderr, "cc-fleet dream morning: %v\n", err)
		return 1
	}
	if _, err := io.WriteString(stdout, output.Stdout); err != nil {
		fmt.Fprintf(stderr, "cc-fleet dream morning: write stdout: %v\n", err)
		return 1
	}
...
func runDreamMigrate(
	...
	flags := newFlagSet(
		...
		"usage: cc-fleet dream migrate-anchors ORGAN",
		stderr,
	)
	if code, ok := parseFlags(flags, args); !ok {
		return code
	...
	if output != "" {
		if _, writeErr := io.WriteString(stdout, output); writeErr != nil {
			fmt.Fprintf(stderr, "cc-fleet dream migrate-anchors: write stdout: %v\n", writeErr)
			return 1
		}
	}
	if err != nil {
		fmt.Fprintf(stderr, "cc-fleet dream migrate-anchors: %v\n", err)
		return 1
	}
	return 0
}
...
func runDreamHook(
	...
	flags := newFlagSet(
		"dream hook",
		"usage: cc-fleet dream hook agent-inject|codex-subagent-inject|nudge",
		stderr,
	)
	if code, ok := parseFlags(flags, args); !ok {
		return code
	...
	if kind != dreamHookNudge {
		...
		if err != nil {
			fmt.Fprintf(stderr, "cc-fleet dream hook %s: read stdin: %v\n", kind, err)
			return 1
		}
		input = payload
	}
	...
	if err != nil {
		fmt.Fprintf(stderr, "cc-fleet dream hook %s: %v\n", kind, err)
		return 1
	}
	if _, err := stdout.Write(output); err != nil {
		fmt.Fprintf(stderr, "cc-fleet dream hook %s: write stdout: %v\n", kind, err)
		return 1
	}
	return 0
}
...
func finishDreamCommand(
	...
	if err != nil {
		fmt.Fprintf(stderr, "cc-fleet dream %s: %v\n", command, err)
		return 1
	}
	if _, err := io.WriteString(stdout, output); err != nil {
		fmt.Fprintf(stderr, "cc-fleet dream %s: write stdout: %v\n", command, err)
		return 1
	}
	return 0
}
...
func printDreamUsage(writer io.Writer) {
	fmt.Fprintln(writer, "usage: cc-fleet dream <command> [options]")
	fmt.Fprintln(writer)
...
func runDreamRestamp(
	...
	flags := newFlagSet(
		"dream restamp",
		"usage: cc-fleet dream restamp maps/{slug}.md   (run from inside the repository)",
		stderr,
	)
	if code, ok := parseFlags(flags, args); !ok {
		return code
	...
	if err != nil {
		fmt.Fprintf(stderr, "cc-fleet dream restamp: resolve working directory: %v\n", err)
		return 1
	}
	output, err := runtime.restamp(flags.Arg(0), workingDirectory, runtime.now())
	if err != nil {
		fmt.Fprintf(stderr, "cc-fleet dream restamp: %v\n", err)
		return 1
	}
	if _, writeErr := io.WriteString(stdout, output); writeErr != nil {
		fmt.Fprintf(stderr, "cc-fleet dream restamp: write stdout: %v\n", writeErr)
		return 1
	}
	return 0
}
