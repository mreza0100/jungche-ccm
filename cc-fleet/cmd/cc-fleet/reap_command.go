...
import (
	...
	"hostops/cc-fleet/internal/reap"
)
...
func runReap(args []string, stdout, stderr io.Writer) int {
	flags := newFlagSet(
		"reap",
		"usage: cc-fleet reap [--apply] [--busy-recent SECONDS]",
		stderr,
	)
	...
	if err != nil {
		fmt.Fprintf(stderr, "cc-fleet reap: %v\n", err)
		return 1
	}
...
func printReapReport(
	...
	if !report.AgentsOK {
		fmt.Fprintf(
			stderr,
			"cc-fleet reap: busy state unknown (%s) — every chat with a breadcrumb is skipped\n",
			report.BusyError,
		)
	}
