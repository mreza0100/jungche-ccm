...
const (
	storeStressEnv         = "CC_FLEET_STRESS"
	storeStressHelperEnv   = "CC_FLEET_STORE_STRESS_HELPER"
	storeStressPrefixEnv   = "CC_FLEET_STORE_STRESS_PREFIX"
	storeStressReadyEnv    = "CC_FLEET_STORE_STRESS_READY"
	storeStressGateEnv     = "CC_FLEET_STORE_STRESS_GATE"
	storeStressCountEnv    = "CC_FLEET_STORE_STRESS_COUNT"
...
func TestStoreStress(t *testing.T) {
	if os.Getenv(storeStressEnv) != "1" {
		t.Skip("set CC_FLEET_STRESS=1 to run the store stress battery")
	}
...
func TestStoreStressHiddenHelper(t *testing.T) {
	...
	if os.Getenv(paths.EnvDB) == "" {
		t.Fatal("store stress helper has no CC_FLEET_DB jail")
	}
