...
19. Which of these, if any?
    ...
    - **Multi-account fleet + `/chat:*`** — only offer if the user runs multiple accounts or several parallel chats; requires Go 1.24+, `tmux`, `zsh`, `fzf`, and a permanent clone. Build `pfm` first; its self-installer embeds the host command cards and helpers. Review `pfm install --dry-run` (the default mode — an apply is explicit), then run `pfm install --apply`. Name the six surfaces it writes before asking for consent: staged assets under `~/.local/share/pfm/install/`, command symlinks into `~/.claude/commands/`, the `pfm-name-sync` scheduler (three systemd user units on Linux, one launchd agent in `~/Library/LaunchAgents/` on macOS), every Claude account settings file it finds (statusline when none is set, plus the usage, group, and `/clear` `SessionEnd` hooks), `~/.codex/hooks.json`, and one source line in `~/.zshrc`. Each rewritten file is backed up first, `pfm install --uninstall` reverses it, and an apply refuses before any write (rc 97) when the host is mid-flight: on Linux a reachable `systemd --user` bus, on macOS a name-sync agent that is currently running. macOS has no dead-bus jail — launchd is always live — so the two gates protect the same moment, not the same condition. Disclose plainly: fleet-launched chats run with permission prompts disabled by design.
