...
framework files should have said; `/pcm retro` consumes it.
...
Resolved:            ← /pcm retro stamps this in place: `Resolved: {date} — {where it landed}`
```

Entries without a `Resolved:` line are the open queue. `/pcm retro` folds each `Amend:` into the
